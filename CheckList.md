...
| `[~]` | NFR-03| 성능    | 앱 cold start → 메인 윈도우 노출            | < 2.0s (M2 MacBook 기준)            | `tools/measure_cold_start.py` (cycle 169.596~631 도구 + probe + dialog skip + codesign chain 4 attempt, source-level 312ms PASS, .app runtime Team ID mismatch 잔존, Apple Developer ID proper sign / macos-15 GitHub-hosted swap 별 cycle 의무) |
