...
## 2. 진행률 요약 (2026-05-24 14:45 KST cycle 169.742 portable harness recovery)
...
| CI 3종 GREEN (ci + docs-lint + doc-gardener) | **PASS** | cycle 169.739 main CI + docs-lint GREEN. cycle 169.742 portable harness PR policy 반영 후 PR CI 재검증 필요 |
...
| Portable Harness 최신화 (거버넌스 + hook + guardrail + trigger + 코드 분리 + skill 분리) | **RECOVERING** | cycle 169.742 — main 직접 push 이식 지시 제거 + feature branch/PR policy 반영 + portable harness meta gate 추가 |
...
| `[~]` | FR-10 | 시그널링 단절 자동 재연결          | signaling_client backoff + ConnectionState chain                                                                 | [app/net/signaling_client.py](app/net/signaling_client.py)                                      | binding / chaos test 부재 |
