...
| `[~]` | NFR-03| 성능    | 앱 cold start → 메인 윈도우 노출            | < 2.0s (M2 MacBook 기준)            | `tools/measure_cold_start.py` (cycle 169.596~616 도구 + probe hook + dialog skip, source-level 312ms PASS, PyInstaller .app crash 식별 — macOS 26.4 + PyQt6 Qt framework init 안 SIGSEGV `_GLOBAL__sub_I_qdarwinpermissionplugin_location.mm`, codesign + spec rpath refactor 별 cycle) |
