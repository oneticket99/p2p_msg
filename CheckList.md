...
- 변경 이력 prepend → `History.md` (운영 8 — 작성 완료)

---

## 2. 진행률 요약 (2026-05-24 15:07 KST cycle 169.744 checkout v5)
...
| 검증 게이트 self-consistency 회수 | **RECOVERING** | cycle 169.739 — latest cycle marker README/History 반영 검사 유지 + auto-commit hook 추적/Stop 연결/main 직접 push 안내 금지 검사 추가 |
| Portable Harness 최신화 (거버넌스 + hook + guardrail + trigger + 코드 분리 + skill 분리) | **RECOVERING** | cycle 169.743 — meta_enforce 작성 가이드(골격 + 필수 check + CI 연결) 편입 + guide token meta gate 추가 |
| 사용자 manual visual ack (10 항목) | **PENDING** | A 1~10 재 빌드 후 사용자 직접 verify |

> Phase 1 MVP DoD 정합은 기능 테스트 누계와 별개로 최신 CI workflow success 를 함께 요구한다.
> cycle 169.739 현재 로컬 M3 가드레일과 CI M3 호출 기준은 `python3` 단일 검증기로 통합됐다.
> 최신 cycle marker 문서 freshness 와 doc-gardener 자동 push/PR 경로도 meta-enforcement 대상이다.
> 원격 CI success 확인 전 `CI 3종 GREEN` 표기는 금지한다.
...
| FR P0/P1              | 9 / 13 done · 0 partial | FR-01~10 모두 done · FR-11/12/13 회원가입/로그인 binding 완료 (cycle 169.621 image E2E PASS) |
| NFR                   | 1 / 7 done · 5 partial | NFR-05 done · NFR-01/02/04/06 측정 인프라 신설 (cycle 169.592~595) · NFR-03 도구 신설 + probe hook (cycle 169.596~597) · NFR-07 manual 캡처 보류 |
| 마일스톤              | M1/M2/M3/M4/M5 done (5/5) | Phase 1 MVP 마일스톤 전체 sign-off (cycle 169.621 image E2E PASS 후 M3/M4 [x]) |
...
| 상태 | ID    | 제목                              | 매핑 task                                                                                                       | 코드 위치 (실 트리 노드 — 전수 링크)                              | 상태 라벨   |
|------|-------|-----------------------------------|-----------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------|-------------|
| `[x]` | FR-01 | 시그널링 서버 연결 + JOIN          | cycle 169.538~545 (원격 PASS + e2e 2 PASS)                                                                       | [server/signaling.py](server/signaling.py) · [server/room.py](server/room.py) · [tests/e2e/test_signaling_browser_flow.py](tests/e2e/test_signaling_browser_flow.py) | done        |
| `[x]` | FR-02 | DataChannel 텍스트 송수신          | cycle 153.7 InputBar + ChatSendMixin (169.527) + RoomGroupChat (169.521) + cycle 169.619 browser native RTCPeerConnection E2E PASS (Alice/Bob "hello"→"world" 왕복) | [app/rtc/peer.py](app/rtc/peer.py) · [app/ui/chat_view.py](app/ui/chat_view.py) · [tests/e2e/test_datachannel_browser_flow.py](tests/e2e/test_datachannel_browser_flow.py) | done |
| `[x]` | FR-03 | 이미지 송수신 (썸네일 + 원본)      | image_processor + message_bubble PIL bbox + cycle 169.621 browser DataChannel image envelope E2E (header JSON + 8KB thumb + 256KB 원본) | [app/rtc/file_sender.py](app/rtc/file_sender.py) · [app/rtc/image_processor.py](app/rtc/image_processor.py) · [app/ui/message_bubble.py](app/ui/message_bubble.py) · [tests/e2e/test_datachannel_image_flow.py](tests/e2e/test_datachannel_image_flow.py) | done |
| `[x]` | FR-04 | 파일 송수신 + 양방향 ProgressBar   | file_sender + file_receiver + file_progress_widget + cycle 169.620 browser native DataChannel 1MB chunk E2E + fingerprint verify | [app/rtc/file_sender.py](app/rtc/file_sender.py) · [app/rtc/file_receiver.py](app/rtc/file_receiver.py) · [app/ui/file_progress_widget.py](app/ui/file_progress_widget.py) · [tests/e2e/test_datachannel_file_flow.py](tests/e2e/test_datachannel_file_flow.py) | done (1MB chunked PASS, 100MB throughput = NFR-02 135 Mbps) |
| `[x]` | FR-05 | 메시지 영속화 (MariaDB)            | mariadb 11 + 9 table + bcrypt 12 + cycle 169.502 bot user ensure + cycle 169.598 클라 SQLite cache binding verify (insert_message + list_messages_by_room 26 call site) | [server/db/](server/db/) · [app/db/messages_cache.py](app/db/messages_cache.py)                | server PASS + 클라 SQLite cache active (bot/chat_helper/chat_send mixin 안 binding) |
| `[x]` | FR-06 | peer 연결 상태 표시                | StatusBar + ChatHeader online/last_active + cycle 169.221 last_seen REST                                         | [app/ui/status_bar.py](app/ui/status_bar.py) · [app/ui/chat_header.py](app/ui/chat_header.py)                    | done        |
| `[x]` | FR-07 | 방 입장 / 퇴장                     | cycle 142 group room + RoomGroupChatMixin (169.521) + JOIN/LEAVE protocol PASS                                   | [server/room.py](server/room.py) · [app/ui/_room_group_chat_mixin.py](app/ui/_room_group_chat_mixin.py) | done        |
| `[x]` | FR-08 | PyInstaller 빌드 (mac+win zip)     | cycle 169.547 build artifact 2 platform PASS (runId 26320924166, macOS 343.8MB + Windows 101.5MB)                | [tools/build.py](tools/build.py) · [.github/workflows/build.yml](.github/workflows/build.yml)            | done        |
| `[x]` | FR-09 | 첫 실행 onboarding                 | WelcomeDialog + SignupDialog + LoginDialog (cycle 169.6~12 mascot 합성)                                          | [app/ui/welcome_dialog.py](app/ui/welcome_dialog.py) · [app/ui/signup_dialog.py](app/ui/signup_dialog.py) · [app/ui/login_dialog.py](app/ui/login_dialog.py) | done        |
| `[x]` | FR-10 | 시그널링 단절 자동 재연결          | signaling_client backoff + reJOIN + RECONNECTING 상태 (cycle 169.775 구현)                                        | [app/net/signaling_client.py](app/net/signaling_client.py)                                      | 지수 backoff + reJOIN 복구 + 통합 test 9 PASS (test_signaling_reconnect). 실 server 대상 e2e 잔존 |
...
> "코드 위치" 컬럼이 실제 트리 노드 링크로 갱신 완료 (cycle 169.816 per-file 감사).
...
| 상태 | ID    | 분류    | 지표                                        | 목표값                             | 측정 도구 / 위치                              |
|------|-------|---------|---------------------------------------------|-------------------------------------|------------------------------------------------------|
| `[~]` | NFR-01| 성능    | 1:1 텍스트 메시지 왕복 RTT (LAN)            | 평균 < 100ms · 95p < 200ms          | `tools/bench_rtt.py` (cycle 169.592 PASS — 원격 demo signaling avg 16.78ms / 95p 78.54ms, 20 sample) |
| `[x]` | NFR-02| 성능    | 100MB 파일 송신 throughput (LAN)            | ≥ 30Mbps 평균                       | `tools/bench_datachannel.py` (cycle 169.617 — aiortc 양 peer DataChannel 직결 10MB = 135.28 Mbps PASS, gate 30Mbps 4.5배 초과) + `tools/bench_transfer.py` (cycle 169.593 signaling relay reference 22.24 Mbps) |
| `[~]` | NFR-03| 성능    | 앱 cold start → 메인 윈도우 노출            | < 2.0s (M2 MacBook 기준)            | `tools/measure_cold_start.py` source-level smoke 312ms PASS (cycle 169.600). PyInstaller .app codesign production = 비의무 (cycle 169.648 [[project-demo-phase-functional-distribution]]). 단 **데모 phase 도 기능적 .app 실행 의무** — Team ID mismatch crash 회수 path 4 (self-extract / LC_RPATH refactor / nuitka / --onefile) 별 cycle. |
| `[~]` | NFR-04| 가용성  | 시그널링 단절 후 자동 재연결 성공률         | 30초 안 99% 이상                    | `tools/chaos_signaling.py` (cycle 169.594 PASS — demo 100% success · 평균 0.028s · 95p 0.087s, 10 cycle) |
| `[x]` | NFR-05| 보안    | 시그널링 envelope 외부 입력 검증             | 화이트리스트 5종 외 거부             | `tests/server/test_protocol.py` + `tests/e2e/test_signaling_browser_flow.py` (cycle 169.539 PASS — UNKNOWN_TYPE/NOT_JOINED ERROR envelope verify) |
| `[~]` | NFR-06| UX      | ProgressBar 진행률 갱신 빈도                | 100ms 안 1회 이상 (정지·역행 0)     | `tools/measure_progress.py` (cycle 169.595 PASS — offscreen 100 iter avg 13.48ms · 역행 0, qt-bot fixture 별 cycle) |
...
| 상태  | ID | 목표일       | 제목                                           | 산출물 요약                                                         |
|-------|----|--------------|------------------------------------------------|---------------------------------------------------------------------|
| `[x]` | M1 | 2026-05-24   | 부트스트랩 + 정책 문서 + 에이전트 정의          | 루트 18 문서 · `docs/` 골격 · `.claude/agents/` process agent + ssh-deploy + dereliction-detector · CI 3 워크플로우 + meta-enforcement L5 |
| `[x]` | M2 | 2026-05-24   | 시그널링 서버 + PyQt 스켈레톤                  | `server/signaling.py` (5 envelope JOIN/OFFER/ANSWER/ICE/LEAVE) · `app/main.py` · qasync 통합 · MainWindow 21 mixin + 9 init helper |
| `[x]` | M3 | 2026-05-24   | WebRTC DataChannel 텍스트 송수신                | `app/rtc/peer.py` · Offer/Answer/ICE 교환 · MariaDB 영구화 + cycle 169.619 browser native RTCPeerConnection DataChannel 텍스트 E2E PASS (Alice/Bob hello/world 왕복) |
| `[x]` | M4 | 2026-05-24   | 파일 / 이미지 송수신 + 양방향 ProgressBar       | `app/rtc/file_sender.py` · `file_receiver.py` · `image_processor.py` + cycle 169.620 file 1MB chunk E2E + cycle 169.621 image envelope E2E + cycle 169.617 DataChannel 직결 135 Mbps PASS + cycle 169.595 ProgressBar 갱신 빈도 13.48ms PASS |
| `[x]` | M5 | 2026-05-24   | PyInstaller 빌드 + GitHub Actions + README     | `tools/build.py` · `.github/workflows/build.yml` · README · cycle 169.547 build artifact 2 platform PASS |
...
- [x] History.md — 개발 히스토리 (M3 — 역순 prepend 전용)
- [x] README.md — 빌드 / 실행 안내 + 변경 이력 30행 (M2 정합)
- [x] EXTENSION_GUIDE.md — 서브에이전트 활용 · 신규 에이전트 도입 절차
- [x] MIGRATION_MARIADB.md — MariaDB 스키마 · FK 정렬 · 마이그레이션 절차
...
마일스톤별 1 CI 게이트. 모두 작성 완료 — 5 / 5. 최신 통과 여부는 workflow run 결과를 별도 확인한다.

- [x] **M1 게이트** — `.github/workflows/ci.yml` "m1-doc-precedence" (코드 변경 PR 문서 동행)
- [x] **M2 게이트** — `.github/workflows/ci.yml` "m2-readme-history" (`README.md` 변경 이력 헤더)
- [x] **M3 게이트** — `.github/workflows/ci.yml` "m3-history-prepend" (History Phase 역순)
- [x] **M4 게이트** — `.github/workflows/ci.yml` "m4-korean-comments" (diff 범위 `.py` · `.js` · `.html` · `.css` · `.sql` · `.sh`)
- [x] **M5 게이트** — `.github/workflows/ci.yml` "import-smoke" + pytest/coverage/e2e 차단 게이트
...
2. **Structure.md** — 코드 위치 컬럼에 신규 파일 등장 시 트리 노드 + 모듈 책임 표 동시 갱신. 본 문서 §3 "코드 위치" 컬럼이 실제 트리 노드 링크로 갱신 완료.
...
4. **History.md** — 본 문서 변경 즉시 History.md (운영 8 — 작성 완료) 상단에 한 줄 prepend (M3 역순).
5. **README.md** — README.md (운영 8 — 작성 완료) `## 변경 이력` 섹션에 한 줄 prepend (M2 · 30행 상한).
