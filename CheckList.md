...
## 2. 진행률 요약 (2026-05-17 기준)

| 영역                  | 완료 / 전체     | 비고                                                              |
|-----------------------|-----------------|-------------------------------------------------------------------|
| 정책 문서 9           | 9 / 9           | AGENTS · ARCHITECTURE · DESIGN · FRONTEND · PLANS · PRODUCT_SENSE · QUALITY_SCORE · RELIABILITY · SECURITY 모두 작성 완료 |
| 운영 문서 8           | 4 / 8           | Specification · Structure · CheckList(본 문서) · CLAUDE_HARNESS_IMPORTANT 완료. History · README · EXTENSION_GUIDE · MIGRATION_MARIADB 작성 예정 |
...
| FR P0/P1              | 0 / 10          | FR-01 ~ FR-10 — 코드 진입 전 (M1 단계)                             |
| NFR                   | 0 / 7           | NFR-01 ~ NFR-07 — 측정 인프라 미구축                               |
| 마일스톤              | M1 in_progress  | M2 ~ M5 pending (실행계획 §4 정합)                                 |
| CI 게이트 5           | 0 / 5           | M1~M5 ci.yml · docs-lint.yml · doc-gardener.yml 작성 예정          |
...
| `[ ]` | NFR-05| 보안    | 시그널링 envelope 외부 입력 검증             | 화이트리스트 5종 외 거부             | `tests/server/test_protocol.py` (예정)               |
...
- [ ] **TBD-01** — 라이선스 종류 (MIT / Apache-2.0 / GPL-3.0 / 사유). [AGENTS.md §1](AGENTS.md) "미확정" 상태와 동일. 사용자 응답 대기.
...
- [ ] **TBD-06** — `.github/workflows/build.yml` self-hosted runner 등록 (macOS arm64 + Windows x64). 사용자가 직접 runner 등록 후 GitHub Settings 에 라벨 부여.
