...
| `[~]` | NFR-03| 성능    | 앱 cold start → 메인 윈도우 노출            | < 2.0s (M2 MacBook 기준)            | `tools/measure_cold_start.py` (cycle 169.596~614 도구 + probe hook stdout + log file dual-path, source-level smoke 312ms PASS, PyInstaller .app rebuild OK, probe path 도달 부재 (LoginDialog modal stuck) — main() offscreen-aware dialog skip refactor 별 cycle) |
