...
| `[~]` | NFR-03| 성능    | 앱 cold start → 메인 윈도우 노출            | < 2.0s (M2 MacBook 기준)            | `tools/measure_cold_start.py` (cycle 169.596~608 도구 + probe hook + source-level smoke 312ms PASS, build artifact 다운 OK, PyInstaller windowed mode stdout 차단 → log file fallback 별 cycle) |
