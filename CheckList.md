...
| `[~]` | NFR-03| 성능    | 앱 cold start → 메인 윈도우 노출            | < 2.0s (M2 MacBook 기준)            | `tools/measure_cold_start.py` (cycle 169.596~628 도구 + probe hook + dialog skip + post-build codesign retain, source-level 312ms PASS, .app runtime fail — self-hosted runner Apple sealed Python framework + PyInstaller bootloader Team ID mismatch, brew Python 또는 macos-15 runner swap 별 cycle) |
