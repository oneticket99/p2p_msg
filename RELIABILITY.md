...
flowchart TB
    ...
    C2 -->|재기동 후 SQLite WAL 복구| RECOVER
...
| Disk full | SQLite `disk I/O error` 또는 `OSError: ENOSPC` | 신규 메시지 큐잉 정지 + 토스트 | 사용자가 정리 후 재시도 | 1 |
| 클라이언트 크래시 | 다음 기동 시 비정상 종료 감지 (lock 파일) | SQLite WAL 자동 회복 | 마지막 송신 메시지 ack 재확인 | 1 |
...
- 모든 메시지·파일 메타 쓰기는 **단일 트랜잭션** 안에서 수행된다.
- WAL 모드 활성화 (`PRAGMA journal_mode=WAL`) — 크래시 시 자동 회복 보장.
- 로컬 DB 경로는 `.env` 의 `SQLITE_PATH` 로만 지정 (하드코딩 금지, [AGENTS.md](AGENTS.md) 부록 B).
...
| 로컬 DB | SQLite WAL + 단일 트랜잭션 | 자동 복구 (WAL replay) |
...
> Phase 1 은 로컬 SQLite 만 사용하며 별도 백업 정책이 없다. 사용자가 직접 DB 파일을 백업해야 한다.

### 7.1 SQLite dump 자동화 (Phase 2)

- 클라이언트가 일일 1회 (애플리케이션 종료 시) `.sqlite` → `.sqlite.dump` 로 `VACUUM INTO` 백업.
...
| 손상 유형 | 복구 절차 |
|---|---|
| WAL 파일만 손상 | 앱 재기동 시 SQLite 가 자동 회복 — 사용자 개입 불필요 |
| 메인 DB 파일 손상 | 백업 디렉토리에서 가장 최근 `.sqlite.dump` 복원 + 사용자에게 안내 |
...
|---|---|---|---|
| 정전 (kill -9) | `kill -9` 클라이언트 PID | 재기동 후 SQLite WAL 자동 회복 + 마지막 송신 메시지 ack 재확인 | 2 deferred |
...
3. 앱 재기동 → SQLite WAL replay 자동 수행
