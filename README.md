...
- [2026-05-17 16:15:00] fork PR 승인 정책 strict 적용 (Task #4) — gh API 측 `PUT /repos/.../actions/permissions/fork-pr-contributor-approval` 의 `approval_policy=all_external_contributors` 자동 적용. 직전 default `first_time_contributors` → 모든 외부 contributor 의 maintainer approval 의무. public repo + self-hosted runner 의 보안 hardening 정합. ci-self-hosted-setup.md §5.1 의 자동 적용 명령 명시 + 정책 옵션 표 + manual UI 대안. CheckList §2 의 신규 row "fork PR 승인 정책 = strict 적용" 추가 (docs/references/ci-self-hosted-setup.md + CheckList.md)
