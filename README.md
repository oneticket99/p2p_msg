...
- [2026-06-09 16:43:00] cycle 169.862 — **webmail 메일 헤더/본문 parse 회수 — INBOX from/subject + HTML entity unescape + 사용자 인계 handoff doc**. 사용자 visual 발견 = 2 bug: (1) INBOX "(unknown sender) (no subject)" — ENVELOPE byte tuple 휴리스틱 `_extract_field` fail 정합 (cycle 169.861 backlog). (2) 메일 본문 안 `&nbsp;` raw entity 출력 (HTML body 직접 노출). 회수: `imap_service.py` (a) `list_inbox` IMAP FETCH 명령 `ENVELOPE` → `BODY.PEEK[HEADER.FIELDS (FROM SUBJECT DATE)]` 변경 + `_parse_header_fields` 신설 (email.message_from_bytes RFC822 정확 parse + `_decode_header_value` MIME encoded-word `=?UTF-8?B?...?=` decode). (b) `_parse_message` 본문 추출 = text/plain 우선 + html.unescape, fallback text/html → `_HTMLToText` html.parser convert_charrefs=True + tag strip + `<script>/<style>` 차단 (XSS 우려, M6 안 bleach 도입 예정). (c) 본 cycle = `_decode_header_value` 호출 추가로 From/To/Subject 헤더 안 한국어 MIME encoded-word 자동 디코드. e2e 14 test PASS (기존 11 + Test 12 `_decode_header_value` MIME UTF-8 base64 + Test 13 `_HTMLToText` &nbsp; unescape + `<script>` 차단 + Test 14 `_parse_header_fields` RFC822 정합). doc-lint PASS + M3 PASS. **사용자 인계 handoff doc 신설** `docs/exec-plans/active/2026-06-09-session-handoff-cycle169.862.md` — main HEAD + webmail/Dovecot 인프라 상태 + 잔존 백로그 (M5 SMTP 발신 + M6 첨부/folder/HTML render bleach) + 가드레일 + SSH/계정 자격 + 즉시 시작 가능 명령. (server_webmail/imap_service.py · tests/integration · docs/exec-plans/active)
