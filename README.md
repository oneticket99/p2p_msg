...
- cycle 169.528 (2026-05-23 05:20 KST) — codex 2.5 main_window 책임 분리 13차 batch — `app/ui/_dialog_center_mixin.py` 신설 (DialogCenterMixin: _exec_dialog_centered 단독 193 line big block 분리, backdrop dim + child overlay + ESC handler + splitter sizes snapshot/restore + chat_list_panel active_tab snapshot/restore + signal accept/reject chain + QDialog bound method fallback) + `app/ui/_menu_actions_mixin.py` 신설 (MenuActionsMixin: _on_open_friend_list + _on_open_direct_chat + _on_open_settings_dialog + _on_open_room_dialog + _on_show_about, 5 method 122 line) + MainWindow MRO 18 mixin + main_window.py 1027 → 786줄 (241줄 분리, 누계 3240줄 4026 → 786, **80.5%**)
...
- cycle 169.509 (2026-05-23 01:15 KST) — codex 2.5 HIGH 진입 — main_window 4000줄 책임 분리 1차 — `app/ui/_tray_mixin.py` 신설 (TrayMixin: _setup_tray_icon + _on_tray_* 4 + _perform_logout_and_relogin, 6 method) + MainWindow MRO `(TrayMixin, QMainWindow)` 상속 + main_window.py 4026 → 3862줄 (164줄 분리)
