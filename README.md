...
- [2026-05-17 16:35:00] docs/references/smtp-setup.md 신설 — SMTP 서버 데모 호스트 (114.207.112.73) 설치 절차 (사용자 directive 2026-05-17 "smtp 서버는 사전에 명시했던 테스트서버에 설치해" + "권장방향 진행해"). 권장 default = postfix (Ubuntu apt) + Let's Encrypt (certbot) + SPF + DKIM (opendkim) + DMARC + aiosmtplib client + SendGrid fallback. 13 섹션 (용도 + 의존성 + DNS + TLS + postfix + DKIM + SASL + 검증 + hardening + fallback + 트러블슈팅 + 운영 체크리스트 + 참조). 영구 메모리 project_smtp_demo_server.md 신설 + MEMORY.md 인덱스 갱신. SSH 차단 상태 → 실제 설치 = 사용자 직접 의무 (docs/references/smtp-setup.md)
- [2026-05-17 16:15:00] fork PR 승인 정책 strict 적용 (Task #4) — gh API 의 `PUT /repos/.../actions/permissions/fork-pr-contributor-approval` + `approval_policy=all_external_contributors` 자동 적용. 직전 default `first_time_contributors` → 모든 외부 contributor 의 maintainer approval 의무. public repo + self-hosted runner 의 보안 hardening 정합. ci-self-hosted-setup.md §5.1 의 자동 적용 명령 명시 + 정책 옵션 표 + manual UI 대안. CheckList §2 의 신규 row "fork PR 승인 정책 = strict 적용" 추가 (docs/references/ci-self-hosted-setup.md + CheckList.md)
- [2026-05-17 16:08:00] ci.yml Windows matrix 영구 비활성 주석 명문화 (5/6 file 완료) — 직전 cycle 의 임시 비활성 → 영구 비활성 변경. import-smoke + pytest job 의 2 영역 주석 + 파일 헤더 주석 모두 갱신. Windows 빌드 의무 의 build.yml 위임 명시 (.github/workflows/ci.yml)
- [2026-05-17 16:04:00] CheckList §2 진행률 표 갱신 (4/6 file) — self-hosted runner 1/2 → 1/1 (Windows runner 의무 영구 회수). CI 워크플로우 row 의 Windows matrix 영구 비활성 + wine 대체 명시. Windows 빌드 패턴 row 신규 (wine docker — cdrx/pyinstaller-windows) (CheckList.md)
- [2026-05-17 16:01:00] handoff doc §5 정책 표 + §9 task #1 갱신 (3/6 file) — wine cross-compile 정합. §5 빌드 row + CI row 의 듀얼 트랙 명시 (self-hosted macOS arm64 + GitHub-hosted Ubuntu + Windows self-hosted 영구 회수). §9 task #1 = ✅ 완료 (Windows runner 의무 회수 + wine 대체). §1 TL;DR 의 CI 영역 갱신 (docs/exec-plans/active/2026-05-17-session-handoff.md)
- [2026-05-17 15:58:00] AGENTS.md §1 빌드 row 갱신 (2/6 file) — macOS arm64 native (self-hosted) + Windows x64 wine cross-compile (GitHub-hosted Ubuntu + cdrx docker) 의 듀얼 트랙 명시. 사용자 directive 2026-05-17 정합 (AGENTS.md)
