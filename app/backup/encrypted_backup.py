...
- ``derive_backup_key`` — HKDF-SHA256 password + salt → 32-byte AES-256 key
...
- PBKDF2 password stretching (slow KDF + brute-force 방어)
...
_BACKUP_VERSION: Final[str] = "1"
# HKDF derive 의 info 의 의 context binding (다른 derive 의 collision 차단)
_HKDF_INFO: Final[bytes] = b"tootalk-backup-key-v1"
# salt 길이 = 16 byte (Signal Protocol HKDF 의 권장 의 의무)
...
def derive_backup_key(password: str, salt: bytes) -> bytes:
    """master password + salt → 32 byte AES-256 backup key.

    Notes
    -----
    HKDF-SHA256 직접 사용 — PBKDF2 stretching 부재 (별개 cycle 의무). 본 cycle =
    Phase 2 minimal skeleton 의 의도. 실 서비스 = PBKDF2 의 600K iteration 의무.
    """
