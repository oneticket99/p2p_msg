...
# cycle 169.358 — labels.tr() 우선 lookup + Qt fallback (사용자 critique English 부재 회수)
...
def _tr(src: str) -> str:
    ...
    cur_loc = _i18n_labels.get_locale()
    val = _i18n_labels.tr(slug)
    log.warning(
        "[i18n_login_dbg] locale=%s src=%r slug=%r val=%r",
        cur_loc, src, slug, val,
    )
    if val != slug:
        return val
