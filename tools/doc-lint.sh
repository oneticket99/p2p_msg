...
# 검증 항목:
#   1. 깨진 상대 markdown 링크
#   2. docs/** frontmatter 필수 필드 (title · owner · last_verified · status)
#   3. 연속 빈 줄 (3 줄 이상 연속 공백 줄 차단)
#   4. BPE 위생 — 한국어 의존명사 "측" 단독 사용 금지
#
# 사용:
...
# ── 검사 1: BPE 위생 — "측" 단독 사용 ────────────────────────────────────────
echo ""
echo "[1/4] BPE 위생 검사 (한국어 의존명사 '측' 단독 사용 금지)"
for f in "${FILES[@]}"; do
...
echo ""
echo "[3/4] docs/** frontmatter 필수 필드 검사 (title · owner · last_verified · status)"
for f in "${FILES[@]}"; do
    # docs/ 하위 파일만 대상
    if [[ "$f" != *"/docs/"* ]]; then
        continue
...
# ── 검사 4: 연속 빈 줄 (3 줄 이상) ───────────────────────────────────────────
echo ""
echo "[4/4] 연속 빈 줄 검사 (3 줄 이상 공백 줄 차단)"
for f in "${FILES[@]}"; do
