...
echo ""
echo "[1/5] BPE 위생 검사 (U+CE21 단독 + 의 3회 반복 + caveman thinking 누설 패턴)"
for f in "${FILES[@]}"; do
    # 1-1. U+CE21 단독 — 합성어 (측면 / 관측 / 측정 / 좌측 / 우측 / 추측) 제외
    HITS=$(grep -nE "(^|[^가-힣])측($|[^가-힣면관추좌우정])" "$f" 2>/dev/null || true)
    if [ -n "$HITS" ]; then
        err "BPE U+CE21 단독 위반 ($f):"
        echo "$HITS" | sed 's/^/        /' >&2
    fi
    # 1-2. 의 단독 조사 3회 반복 — caveman thinking 누설 (사이클 22 직무유기 검출 정합)
    REPEAT=$(grep -nE "의 의 의" "$f" 2>/dev/null || true)
    if [ -n "$REPEAT" ]; then
        err "BPE 의 3회 반복 위반 ($f):"
        echo "$REPEAT" | sed 's/^/        /' >&2
    fi
done
