...
| `server/api/messages_handlers.py` | REST 1 endpoint — GET /api/messages?room_id&start_ts_ms&end_ts_ms&limit (Phase 3 사이클 60 ChatView lazy load server counterpart, `_DEFAULT_LIMIT` 1000 + `_MAX_LIMIT` 5000 의 unbounded SELECT 차단) | `aiohttp` + `server/db/repositories/messages.list_messages_in_range` | 서버 Router REST |
