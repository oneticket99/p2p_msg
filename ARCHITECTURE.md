...
| `app/bot/` | Phase 3 bot framework (사이클 65~71) — `llm_proxy.py` (사이클 65 + 71 — AnthropicProvider 의 AnthropicClient adapter + lazy from_env) + `customer_service_bot.py` (사이클 66 + 69 — RAGStore 통합 의 answer pipeline 의 system prompt augmentation) + `streaming_helper.py` (사이클 67) + `rag_context.py` (사이클 68 + 75 + 79 — FAQEntry + RAGStore Protocol + KeywordRAGStore + Embedder Protocol + MockEmbedder hash-based + CachedEmbedder OrderedDict LRU + cosine_similarity + EmbeddingRAGStore in-memory cosine sim ranking + build_default_toonation_faq 5 영역 10 entry + compose_rag_context markdown) + `anthropic_client.py` (사이클 70 + 77 — Messages API serialize_messages system 분리 + parse_response text block 합본 + AnthropicClient + HttpTransport Protocol + 4 종 예외 auth/rate/server/malformed + retry/backoff + retry-after honor + jitter + ConnectionError/OSError/TimeoutError network retry + from_env factory) | stdlib + (sentence-transformers + httpx + obs-websocket-py + platform API 별개 cycle) | 클라이언트 Bot Framework (default 투네이션 고객센터 봇 + 방송 도우미 봇 + RAG context + bot ↔ RAG 통합 + Anthropic Messages API client + LLMProvider ↔ Client adapter) |
...
| `server/api/bot_handlers.py` | REST 1 endpoint — POST /api/bot/chat (Phase 3 사이클 74 + 78 server-side LLM proxy + ANTHROPIC_API_KEY 격리 + per-user RateLimitGate + 32 messages cap + 16KB content cap + system role 클라이언트 차단 + bool/float/string user_id reject hardening + 4 종 예외 매핑 auth 500/rate 503/server 502/malformed 502) | `aiohttp` + `app/bot/llm_proxy.py` LLMProvider + `app/bot/anthropic_client.py` 예외 | 서버 Router REST |
