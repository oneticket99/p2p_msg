...
| `app/remote/` | Phase 3 entry 원격 데스크탑 차별화 skeleton (사이클 55~57) — `permission.py` Pattern A/B + `protocol.py` RemoteFrame + RemoteInput + RemoteSession + `capture.py` (사이클 57) CaptureBackend Protocol + MockCaptureBackend + MacOSQuartzBackend placeholder + select_capture_backend factory + captured_to_remote_frame BGRA→RGB 변환 | stdlib + (Phase 3 mid 의 PyObjC / ctypes / Xlib 별개 cycle) | 클라이언트 Remote Desktop |
