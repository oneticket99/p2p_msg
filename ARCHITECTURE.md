...
| `app/bot/` | Phase 3 bot framework skeleton (사이클 65) — `llm_proxy.py` BotMessage + BotRole Enum + LLMProvider Protocol + MockLLMProvider (deterministic echo) + AnthropicProvider placeholder + select_llm_provider factory (mock/anthropic/openai/gemini) + RateLimitGate (token bucket 분당 N건 per user_id) | stdlib + (httpx + ANTHROPIC_API_KEY 의 별개 cycle 실 binding) | 클라이언트 Bot Framework (default 투네이션 고객센터 봇) |
