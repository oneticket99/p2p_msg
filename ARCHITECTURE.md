...
| `app/bot/` | Phase 3 bot framework (사이클 65~66) — `llm_proxy.py` (사이클 65 BotMessage + LLMProvider Protocol + Mock + Anthropic placeholder + RateLimitGate) + `customer_service_bot.py` (사이클 66 default 투네이션 고객센터 봇 — CustomerServiceConfig + default_system_prompt 5 영역 (후원/정산/OBS/사기/환불) + truncate_history cap + CustomerServiceBot answer 의 rate limit + history trim + LLM chain) | stdlib + (httpx + ANTHROPIC_API_KEY 의 별개 cycle 실 binding) | 클라이언트 Bot Framework (default 투네이션 고객센터 봇) |
