...
| `app/backup/` | Phase 2 encrypted backup / restore (사이클 48) — `encrypted_backup.py` 의 BackupEntry + BackupBundle + HKDF-SHA256 derive_backup_key + AES-256-GCM encrypt_backup / decrypt_backup + wire format bytes serialize / deserialize | `cryptography>=42.0` (e2ee 재사용) | 클라이언트 Backup |
