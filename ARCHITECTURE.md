...
| 모듈 | 책임 | 의존성 | 위치 |
|---|---|---|---|
| `app/ui/` | PyQt6 위젯·QSS·signup/login/reset dialog (사이클 23) + main_window 계정 메뉴 + **아바타 이미지 picker 파이프라인** (cycle 169.852 — `_avatar_picker_button.AvatarPickerButton` 드롭다운 파일/카메라/클립보드 + `_camera_capture_dialog.CameraCaptureDialog` QtMultimedia in-app 모달 + `_avatar_cache.AvatarCache` 싱글톤 mem+disk 캐시·async fetch·`avatar_ready` signal + `_avatar_helper.make_avatar_pixmap` 표시 전파 6 site, 이니셜 fallback 무손상) | `app/core/` · `app/net/auth_client.py` · `app/net/avatars_client.py` | 클라이언트 UI 계층 |
...
| `app/bot/` | Phase 3 bot framework (사이클 65~81) — `llm_proxy.py` (cycle 65+71 AnthropicProvider adapter) + `customer_service_bot.py` (cycle 66+69+83 RAGStore 통합 + scan_jailbreak opt-in) + `streaming_helper.py` (cycle 67) + `rag_context.py` (cycle 68+75+79 FAQEntry + RAGStore + KeywordRAGStore + Embedder + MockEmbedder + CachedEmbedder LRU + cosine_similarity + EmbeddingRAGStore) + `anthropic_client.py` (cycle 70+72+73+77 Messages API + retry + retry-after + jitter + network retry) + `jailbreak_detector.py` (cycle 81 prompt injection heuristic — 6 category × Korean/English regex + JailbreakSignal NONE/SUSPICIOUS/BLOCKED + severity 누적) + `openai_client.py` (cycle 84 Chat Completions API + Bearer auth + system role inline + choices[0].message parse + 4 종 예외 + retry/backoff/retry-after/jitter + network retry + from_env + OpenAIProvider adapter 의 llm_proxy 통합) + `usage_tracker.py` (cycle 85 UsageRecord + UsageSummary + UsageTracker + per-user/provider/period 집계 + extract_anthropic_usage + extract_openai_usage 의 token 통계) + `escalation_queue.py` (cycle 86 EscalationTicket + TicketStatus 4종 + EscalationReason 6종 + EscalationQueue 의 lifecycle PENDING/ASSIGNED/RESOLVED/CLOSED + enqueue/assign/resolve/close + list_pending/assigned/by_user/by_agent) + `streaming.py` (cycle 87 SSE parser — StreamEvent 7종 + StreamChunk + parse_sse_line + extract_anthropic_delta + extract_openai_delta + accumulate_chunks + is_terminal) | stdlib + (sentence-transformers + httpx + obs-websocket-py + platform API 별개 cycle) | 클라이언트 Bot Framework (default 고객센터 + 방송 도우미 + RAG + Anthropic + OpenAI + adapter + jailbreak + usage + escalation + streaming SSE) |
...
| `server/api/bot_handlers.py` | REST 1 endpoint — POST /api/bot/chat (Phase 3 사이클 74 + 78 + 82 server-side LLM proxy + ANTHROPIC_API_KEY 격리 + per-user RateLimitGate + 32 messages cap + 16KB content cap + system role 클라이언트 차단 + bool/float/string user_id reject hardening + jailbreak detector 통합 (cycle 82 BLOCKED → 400 + SUSPICIOUS log) + 4 종 예외 매핑 auth 500/rate 503/server 502/malformed 502) | `aiohttp` + `app/bot/llm_proxy.py` LLMProvider + `app/bot/anthropic_client.py` 예외 + `app/bot/jailbreak_detector.py` | 서버 Router REST |
...
| `server/db/` | `connection.py` (asyncmy pool + 환경변수 8) + repository (users/email_verification/password_reset/rooms/peers/file_meta/messages/devices + **avatars** content-addressed 디스크 저장 sha256 dedup + path traversal 방어, cycle 169.852 M1) + migrations(`0001_init` ~ `0018_user_avatar_field` users.avatar_ref + `0019_room_kind_channel` rooms.kind ENUM channel) (필드 COMMENT 5요소 의무) | `asyncmy>=0.2.10` · `Pillow` (avatar 512 crop+EXIF strip) | 서버 영속화 |
| `server/api/avatars_handlers.py` | REST 3 endpoint — POST /api/avatars(multipart 업로드 + Pillow 정사각 512 center crop + EXIF strip) + GET /api/avatars/{filename} + PATCH /api/me/avatar (cycle 169.852 M2) | `aiohttp` + `Pillow` + `server/db/repositories/avatars.py` | 서버 Router REST |
...
| `server/main.py` | entry + DB pool + auth middleware + session_store + on_cleanup (사이클 22) + bot LLM proxy 통합 (사이클 76 — BOT_ENABLED=1 시 AnthropicProvider 또는 MockLLMProvider 폴백 + RateLimitGate + register_bot_routes + ANTHROPIC_API_KEY 가용성 detect) | 위 + `dotenv` + `signaling_persistence` + `app/bot/llm_proxy.py` | 서버 부트스트랩 |
