...
| `app/bot/` | Phase 3 bot framework (사이클 65~67) — `llm_proxy.py` (사이클 65) + `customer_service_bot.py` (사이클 66 default 투네이션 고객센터 봇) + `streaming_helper.py` (사이클 67 방송 도우미 봇 별개 API — StreamingPlatform 5 종 (YouTube/Twitch/CHZZK/Kick/OBS_LOCAL) + StreamingBotConfig (bot_user_id ≥ 2_000_000 prefix 분리) + StreamingCommand (trigger ! prefix + response template + cooldown) + StreamingHelperBot (apply_command + add/remove + find_command) + default 5 명령 (!hello / !uptime / !donate / !command / !so) + fetch_platform_callback placeholder) | stdlib + (httpx + ANTHROPIC_API_KEY + obs-websocket-py + YouTube Data API + Twitch IRC + CHZZK API + Kick API 의 별개 cycle) | 클라이언트 Bot Framework (default 투네이션 고객센터 봇 + 방송 도우미 봇) |
