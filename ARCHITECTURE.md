...
| `app/remote/` | Phase 3 entry 원격 데스크탑 차별화 skeleton (사이클 55) — `permission.py` 의 Pattern A (도움-mode) / Pattern B (제어-mode) PermissionRequest + PermissionGrant + revoke_token + `protocol.py` 의 RemoteFrame (3 format) + RemoteInput (4 event type) + RemoteSession wire format | stdlib (secrets / dataclass) — platform-specific capture = 별개 cycle | 클라이언트 Remote Desktop |
