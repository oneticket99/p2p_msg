...
[2026-05-17 14:44:47] CLAUDE_HARNESS_IMPORTANT.md 정본 BPE 25건 일괄 정정 (handoff §9 우선순위 2번 잔존 task 해소). sed " 측 " → " 의 " bulk 적용. 정본 본문 의미 보존 (가드레일 [[feedback-no-korean-chuck-token]] 정합 — 합성어 측면/관측/측정 등 보존). doc-lint.sh 5 검사 PASS. 본 사이클 BPE 회수 누계 ~245건 (auth 5 + handoff + assessments + 정본 25) (커밋 대기)
...
[2026-05-17 13:40:44] AGENTS.md PR 게이트 체크리스트 — build.yml (M5 PyInstaller 매트릭스) 행 추가 + Phase 1 active 명시. CI 워크플로우 3 → 4 (build.yml = Phase 1 후반 신설 예정). 실행계획 §6 측 build.yml 등재 정합 (b2b5bcb)
...
[2026-05-17 13:06:07] FRONTEND.md + FRONTEND.html 색상 swatch 가시화 (사용자 directive 2026-05-17). 색상 변수 표 §4 측 9 hex 변수 (bg/fg/bubble-self/bubble-other/bubble-border/text-timestamp/text-sender/status-connected/status-error) 라이트+다크 18개 hex 값에 14px 색상 swatch 추가. FRONTEND.md inline HTML span (style background) + FRONTEND.html .swatch CSS 클래스 (head style 추가). 미확정 후크 항목 (--primary / --progress-acked / --progress-inflight) 측 swatch 미추가 (값 미정). .markdownlint.json MD033 allowed_elements 측 span + div 추가 (CLAUDE.md §10-6 동시 갱신 의무 정합) (커밋 대기)
