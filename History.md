...
[2026-05-17 12:50:22] tools/doc-lint.sh bash 3.2 호환 fix + 1인칭/3인칭 검사 5번 추가. mapfile → while read fallback (macOS 기본 bash 3.2 측 즉시 실행 가능). 검사 4종 → 5종 확장 (BPE + 깨진 링크 + frontmatter + 연속 빈 줄 + 1인칭/3인칭 대명사). 가드레일 자동화 강화 — feedback-no-self-other-pronoun 자동 grep 검출. 전수 검사 가동 검증 OK — 잔존 15 위반 발견 (BPE 3 + 깨진 링크 12) 별도 task 등록 (커밋 대기)
