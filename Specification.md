...
| FR-05 | 메시지 영속화 (MariaDB)              | 대화·이미지·파일 메타데이터를 로컬 MariaDB 에 저장, 앱 재실행 시 이전 히스토리 복원                         | P0       | M3    |
...
| FR    | 실행계획 task                                                                                              | 코드 위치 (예정)                                       | CheckList 항목 (작성 예정 — 운영 2/8) |
|-------|------------------------------------------------------------------------------------------------------------|--------------------------------------------------------|---------------------------------------|
| FR-01 | [#14 server/signaling.py](docs/exec-plans/active/2026-05-17-tootalk-phase1-mvp.md)                         | [server/signaling.py](server/signaling.py)             | CL-01 (작성 예정 — 운영 2/8)          |
| FR-02 | [#17 app/rtc/peer.py](docs/exec-plans/active/2026-05-17-tootalk-phase1-mvp.md) · #18 채팅뷰 wiring         | `app/rtc/peer.py` · `app/ui/chat_view.py` (예정)       | CL-02 (작성 예정 — 운영 2/8)          |
| FR-03 | [#19 transfer.py](docs/exec-plans/active/2026-05-17-tootalk-phase1-mvp.md) · #20 썸네일                    | `app/rtc/transfer.py` · `app/ui/message_bubble.py` (예정) | CL-03 (작성 예정 — 운영 2/8)          |
| FR-04 | [#19 청크 스트림](docs/exec-plans/active/2026-05-17-tootalk-phase1-mvp.md) · #20 ProgressBar               | `app/rtc/transfer.py` · `app/ui/file_progress_widget.py` (예정) | CL-04 (작성 예정 — 운영 2/8)   |
| FR-05 | [#16 db_init.py](docs/exec-plans/active/2026-05-17-tootalk-phase1-mvp.md) · #18 영구화                     | `app/core/storage.py` · `tools/db_init.py` (예정)      | CL-05 (작성 예정 — 운영 2/8)          |
| FR-06 | [#15 main_window.py](docs/exec-plans/active/2026-05-17-tootalk-phase1-mvp.md)                              | `app/ui/status_bar.py` (예정)                          | CL-06 (작성 예정 — 운영 2/8)          |
| FR-07 | [#14 signaling.py](docs/exec-plans/active/2026-05-17-tootalk-phase1-mvp.md) · #15 main_window               | `server/room.py` · `app/ui/main_window.py` (예정)       | CL-07 (작성 예정 — 운영 2/8)          |
| FR-08 | [#21 build.py](docs/exec-plans/active/2026-05-17-tootalk-phase1-mvp.md) · #22 build.yml                    | `tools/build.py` · `.github/workflows/build.yml` (예정) | CL-08 (작성 예정 — 운영 2/8)          |
| FR-09 | [#15 main_window.py](docs/exec-plans/active/2026-05-17-tootalk-phase1-mvp.md)                              | `app/ui/onboarding_dialog.py` (예정)                   | CL-09 (작성 예정 — 운영 2/8)          |
| FR-10 | [#14 signaling reconnect](docs/exec-plans/active/2026-05-17-tootalk-phase1-mvp.md)                         | `app/net/signaling_client.py` (예정)                   | CL-10 (작성 예정 — 운영 2/8)          |

> `Structure.md` (작성 예정 — 운영 2/8) 가 완성되면 본 표의 "코드 위치 (예정)" 열은 실제 트리 노드 링크로 갱신한다.
...
4. **History.md** — 본 문서 변경 즉시 History.md (작성 예정 — 운영 2/8) 상단에 한 줄 prepend (M3 역순).
5. **README.md** — README.md (작성 예정 — 운영 2/8) `## 변경 이력` 섹션에 한 줄 prepend (M2, 30행 상한).
...
- **TBD-01**. 라이선스 종류 (MIT / Apache-2.0 / GPL-3.0 / 사유) — AGENTS.md §1 "미확정" 상태와 동일. 사용자 응답 대기.
...
- Structure.md (작성 예정 — 운영 2/8) — 파일 트리 + ERD (FR ↔ 코드 매핑 정합).
- CheckList.md (작성 예정 — 운영 2/8) — 항목 추적 · 매핑 · 진행률.
- History.md (작성 예정 — 운영 2/8) — 개발 히스토리 (역순 prepend).
- README.md (작성 예정 — 운영 2/8) — 빌드/실행 안내 + 변경 이력 30행.
