...
- **badge 즉시 갱신 결선 확인 (M3)** — by-username 발신 후 발신자 쪽 화면에는 badge 변화 없음(수신자만 badge). 수신자 쪽은 기존 `_refresh_pending_badge` (로그인 직후 + resolve 직후 + polling) 가 자동 처리됨을 e2e/offscreen 으로 확인. 신규 위젯·신규 호출 추가 없이 기존 결선의 정상 동작 검증.
...
| G3     | `contacts_dialog` by-username 성공 시 "친구 요청 발신" 안내 + 즉시 친구목록 추가 부재 + 수신자 쪽 badge count 가 list_pending 길이로 갱신됨을 offscreen Qt + e2e 로 확인 + PendingRequestsDialog/badge 기존 test 무손상 + `pytest tests/` 무손상 + **사용자 GO/NO-GO 응답** | NO-GO → M4 보류 + 잔존 TD 등재 |
