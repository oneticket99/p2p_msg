...
| T-7 | M6 | `app/ui/_*_mixin.py` filler `한글 주석` 패턴 → 의도 기반 전환 + signal/slot 계약 | main session | T-6 | offscreen pytest 무변경 + filler 카운트 감소 + diff 주석만 | `app/ui/_*_mixin.py` (22) | ✅ M6 완료 51/51 (mixin 22/22 + dialog 29/29) · 🔄 M7 e2e 9/9 완료 + server 3/48(s1) → app 155 (사용자 directive e2e 우선) |
