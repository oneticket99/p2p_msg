...
| T-7 | M6 | `app/ui/_*_mixin.py` filler `한글 주석` 패턴 → 의도 기반 전환 + signal/slot 계약 | main session | T-6 | offscreen pytest 무변경 + filler 카운트 감소 + diff 주석만 | `app/ui/_*_mixin.py` (22) | 🔄 mixin 6/22 (b1 friend_status·lifecycle·signaling, b2 menu_actions·invite·room_group) |
