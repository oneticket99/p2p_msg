...
### 1.4 사전 의존성 install (Item 1 진입 이전 의무, 사용자 directive 2026-05-22 — "httpx 설치도 계획에 잡아")

Phase 4 진입 이전 의 server-side Python 의존성 install. 사용자 directive 2026-05-22 = "httpx 설치도 계획에 잡아" — Phase 3 bot framework 의 LLM provider HTTP transport (AnthropicClient + OpenAIClient) 의 실 binding 활성 의무.

| 패키지 | 버전 | 용도 | 진입 cycle |
|---|---|---|---|
| `httpx>=0.27` | latest stable | `AnthropicClient.httpx_transport` + `OpenAIClient.httpx_transport` factory 실 binding. 미설치 시 `AnthropicProvider.is_available()` / `OpenAIProvider.is_available()` 모두 False (graceful) → MockLLMProvider 폴백. 프로덕션 배포 의무. | **cycle 100** (Item 1 진입 직전) |
| `firebase-admin>=7.0` | latest stable | Phase 2 사이클 47 `app/notify/push.py` 의 Firebase Cloud Messaging API v1 실 binding. 미설치 시 graceful fallback (log warning + push skip). | cycle 103~104 (firebase-fcm 컨테이너 설정 직전) |
| `obs-websocket-py>=1.0` | latest stable | Phase 3 방송 도우미 봇 (cycle 67 `app/bot/streaming_helper.py`) 의 OBS WebSocket local 통합. Phase 4 범위 외 — Phase 5+ 진입 시 install. | (Phase 5+ 보류) |

**install 의무 순서**:

1. **cycle 100** = `server/requirements.txt` httpx>=0.27 entry 추가 (본 cycle 97 완료) + `pip install -r server/requirements.txt` 의 local + remote 양쪽 실행 + `python -c "import httpx; print(httpx.__version__)"` 의 verify + `pytest tests/server/test_main_integration.py::TestBuildAppBotEnabled::test_anthropic_preferred_over_openai` 의 skip → PASS 전환 검증 (2 skip → 0 skip).
2. **cycle 103~104** = `server/requirements.txt` firebase-admin entry 추가 + Google Cloud Console FCM enable + service-account.json upload + `firebase-admin>=7.0` install + `app/notify/push.py` 의 graceful fallback → 실 binding 전환.
3. **cycle 100 verify gate** — httpx install 후 `pytest tests/server/test_main_integration.py -v` 의 2 skip → 0 skip 전환 의무. skip 잔존 시 install 실패 의 진단 + 재시도.

**보안 의무**:
- `pip install` 의 `--require-hashes` 또는 `pip-compile` 의 lockfile 의 SBOM 의무 (cycle 101 의 hash lock).
- `httpx[http2]` 의 h2 의존성 의 별개 cycle 의무 (HTTP/2 upgrade 의 Anthropic / OpenAI 응답 latency 단축).
- `httpx[socks]` 의 별개 cycle 의무 (방화벽 의 SOCKS proxy 의무 시 활성).
- `firebase-admin` 의 service-account.json 의 git ignore 의무 (root .gitignore + secrets/ 디렉토리 + Docker secret mount).
