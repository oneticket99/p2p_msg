...
### 2.19 자체 drift detect + 회수 누계 패턴 (신규 사이클 8)

본 cycle 의 자율 진행 패턴 = **사용자 IDE open 의 hint 기반 자체 drift detect + 4 cycle 누계 회수**:
...
본 패턴 = **사용자 명시 의 의 의 자율 reasonable call + 정합 검사 의 누계 + 4 cycle 의 단일 directive 의 자체 분해**. 정책 본문 + 운영 문서 의 라이선스/visibility/hook 의 의 의 정합 100% 의 완성.
