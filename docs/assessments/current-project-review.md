...
owner: oneticket99
last_verified: 2026-05-26T12:05:00+09:00
status: active
---

# TooTalk 현재 프로젝트 전면평가

> 검토 기준: 2026-05-26 cycle 169.849 main branch + room broadcast → 통합 ChatView 마이그레이션 M1~M5b 완결 + Codex 평가 §8 직접 작업 큐 회수 진척(§8-1 sqlite ResourceWarning 해소 + §8-3 sfu_call_client coverage 14→89%). 직전 169.841 평가(P0 = legacy room path 전체 마이그레이션)에서 지목한 P0 항목이 cycle 169.842~848 에 단계별(M2 송신 echo → M3 적재 cache → M4 진입 통일 → M5 위젯 회수 → M6 송신 coverage → M5b idx 재번호+파일 삭제) reviewer 게이트 전수 PASS 로 완결됐다.
...
- ✅ (cycle 169.849 진척) SFU coverage — `app/net/sfu_call_client.py` 단위 test 18 PASS 신설로 14.39% → 89.21% (dispatch/dedup/rollback/answer/producers/close 실효 커버, codex §8-3). `server/sfu_room.py` (49%)는 후속.
...
1. coverage omit 축소 1차: ✅ `app/net/sfu_call_client.py` (14→89%, cycle 169.849) / 🟡 `server/sfu_room.py` (49%, 후속 — MediaRelay forward track 단위 test).
...
1. ✅ (cycle 169.849 해소) `sqlite3.Connection` 미종료 `ResourceWarning` — `app/db/local_db.py` 싱글톤이 `get_connection` 단일 소유 + `close_connection` 명시 호출 누락(pytest process 종료) 경로에서 GC `__del__` close 였다. `atexit.register(close_connection)` 추가로 결정적 close 보장(`_conn` 미오픈 None 가드 no-op). messages_cache 는 동일 싱글톤 재사용 → 단일 fix 로 충분.
2. M6 WBS 정책 결정: `data/wbs.sqlite` 가 gitignored local artifact 이므로, 재개/backfill 또는 영구 비활성 중 하나를 사용자에게 확인한 뒤 문서와 hook 정책을 맞춘다. **(사용자 ack 대기)**
3. coverage omit 축소 1차: ✅ `app/net/sfu_call_client.py` 단위 test 18 PASS(14→89%, cycle 169.849 — aiortc mock 으로 dispatch/dedup/rollback/answer/producers/close 실효 커버). 🟡 `server/sfu_room.py`(49%) MediaRelay forward track 단위 test 후속. (sfu_call_client/sfu_room 은 omit list 부재 — integration test deselected 로 저커버였던 것을 default-run 단위 test 로 회수.)
4. 배포 smoke 증거 확보: macOS `.app` 실행, Windows zip 실행, 업데이트 URL dead-path guard 를 release checklist 에 증거 형식으로 남긴다. **(사용자/manual)**
5. `app/i18n/labels.py:68/69/72` 출처 주석 dangling(삭제된 `group_chat_view.py` 가리킴) — `i18n_extract.py` 재추출 cycle 로 분리 (OBS-2). 수동 편집 시 `labels_extract.json` desync + orphan key drop 위험 → 도구 재실행 의무.
