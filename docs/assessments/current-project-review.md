...
YouTube streaming client 테스트는 skeleton 동작으로 `connect() is False`를 기대한다. 구현은 `httpx`가 있으면 `AsyncClient`를 생성하고 `True`를 반환한다. 테스트와 제품 방향 중 하나를 정본화해야 한다.

관련 근거:

- `tests/app/bot/streaming/test_streaming_clients.py`
