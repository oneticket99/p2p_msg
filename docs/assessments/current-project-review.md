...
owner: oneticket99
last_verified: 2026-05-26T16:26:00+09:00
status: active
---

# TooTalk 현재 프로젝트 전면평가

> 검토 기준: 2026-05-26 16:26 KST main branch + room broadcast → 통합 ChatView 마이그레이션 M1~M5b 완결 + Codex 평가 §8 직접 작업 큐 회수 완결(§8-1 sqlite ResourceWarning 해소 + §8-3 SFU coverage sfu_call_client 14→89% + sfu_room 49→100% + M6 WBS 사용자 ack "재개+backfill" 수령·524 row backfill). 직전 169.841 평가(P0 = legacy room path 전체 마이그레이션)에서 지목한 P0 항목이 cycle 169.842~848 에 단계별(M2 송신 echo → M3 적재 cache → M4 진입 통일 → M5 위젯 회수 → M6 송신 coverage → M5b idx 재번호+파일 삭제) reviewer 게이트 전수 PASS 로 완결됐다. 본 16:26 보정은 cycle 169.850 의 sfu_room coverage 완결 + M6 WBS ack/backfill 회수를 반영한다.
...
2026-05-26 16:24 KST 기준 로컬 확인 결과.

- `git log --oneline`: HEAD = `1405a63 fix(cycle169.850): 평가 HTML mirror 주석 미닫힘 회귀 회수 — productization 빈 화면 해소`.
- `.venv/bin/python -m pytest -q`: `2564 passed, 24 skipped, 317 deselected in 7.08s` (기본 unit suite, integration/e2e deselected).
...
- `sqlite3 data/wbs.sqlite 'select count(*) from wbs_tasks;'`: `675`.
...
- `.venv/bin/python -m tools.md_agents`: PASS — M3 history 436 entries, M2 README, root md 18개.
- `git status -sb`: `main...origin/main` + 본 평가 보정 변경 3건(`History.md`, `README.md`, `docs/assessments/current-project-review.md`) + untracked `tests/server/test_sfu_room.py`. untracked test 는 cycle 169.850 SFU coverage 산출로 보이며, Claude 는 커밋/보존 범위를 먼저 확인한 뒤 진행해야 한다.
...
1. `git status -sb`
2. `git log --oneline -8`
3. 본 평가 보정 변경 3건 + untracked `tests/server/test_sfu_room.py` 의 변경 주체와 보존 범위 확인
...
8. P1(배포 산출물 smoke / 원격 M4 실 OS / i18n 재추출 / active plan 정리) 우선순위 진입.

## 8. Claude 진입용 직접 작업 큐

다음 Claude 세션은 신규 기능보다 정합성 마감부터 잡는다. 아래 순서가 그대로 작업 진입 순서다.

1. **Dirty tree 분리** — `git status -sb` 로 본 평가 보정 변경 3건(`History.md`, `README.md`, `docs/assessments/current-project-review.md`)과 untracked `tests/server/test_sfu_room.py` 를 확인. untracked test 는 cycle 169.850 SFU coverage 산출로 보이며, 덮어쓰기 금지. 필요한 경우 diff 를 읽고 별도 commit/보존 계획 수립.
2. ✅ **M6 WBS 정책 결정/ backfill 완료 확인** — `data/wbs.sqlite` 는 16:24 기준 `wbs_tasks` 675 rows. 다음 Claude 작업은 hook 재중단 방지와 신규 directive row 증분 검증만 수행한다.
...
4. **배포 smoke 증거 확보** — macOS `.app` 실행, Windows zip 실행, 업데이트 URL dead-path guard 를 release checklist 에 증거 형식으로 남긴다. manual/사용자 기기 증거가 필요한 항목은 문서에 `MANUAL` 로 분리한다.
5. **i18n dangling source 회수** — `app/i18n/labels.py:68/69/72` 와 `labels_extract.json` 의 삭제된 `group_chat_view.py` 출처 주석을 `tools/i18n_extract.py` 재추출 cycle 로 정리한다. 수동 편집 금지(추출 JSON desync 위험).
6. **active plan 정리** — 완료된 handoff/마이그레이션 plan 은 archive/finalize 정책 확인 후 이동 또는 status 정정. 파일 이동 시 README/History 동시 갱신.
7. ✅ (cycle 169.849 해소) `sqlite3.Connection` 미종료 `ResourceWarning` — `app/db/local_db.py` 싱글톤이 `get_connection` 단일 소유 + `close_connection` 명시 호출 누락(pytest process 종료) 경로에서 GC `__del__` close 였다. `atexit.register(close_connection)` 추가로 결정적 close 보장(`_conn` 미오픈 None 가드 no-op). messages_cache 는 동일 싱글톤 재사용 → 단일 fix 로 충분.
