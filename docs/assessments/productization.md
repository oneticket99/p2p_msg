...
owner: oneticket99
last_verified: 2026-05-25T20:50:00+09:00
status: active
---

> **최신 갱신 시점**: 2026-05-25 20:50 KST — cycle 169.813 refresh — 음성·영상 SFU 그룹 통화 종단 완결(server PR#12 + client PR#13 merge, MainWindow entry 까지) + Codex 전면평가 환류(cycle 811 review cycle 797→810 + §3.6 SFU IMPLEMENTED + check_assessment_consistency [PASS] + 812 Specification/CheckList "작성 예정" 21건 sweep). reviewer-gate 11 feat 전수 PASS. 잔여 = visual ack 후반 일괄 + Codex P0 FR 코드경로 "(예정)" 감사. 종합 7.6/10.<br>2026-05-25 19:30 KST — cycle 169.808 refresh — 음성·영상 SFU 그룹 통화 **전 경로 코드 완결**: server(M3a/b/c, PR#12 main merge) + client net(M4a SfuCallClient 804 + M4b-1 SignalingClient SFU dispatch/send 805) + UI(M4b-2a GroupCallDialog 타일 그리드 806) + 배선(M4b-2b SfuCallMixin 807, PR#13). publish→SFU MediaRelay forward→producers broadcast→auto subscribe→on_remote_track→타일 종단 흐름 reviewer-gate 전수 PASS + headless(aiortc E2E + offscreen Qt) 검증. 잔존 = MainWindow 합성 entry + visual ack 큐(후반 일괄). 종합 7.6/10.<br>2026-05-25 18:50 KST — cycle 169.802 refresh — 음성·영상 SFU 그룹 통화 **server-side 완결** (cycle 798 M3a protocol SFU 타입 → 799 M3b sfu_room.py aiortc MediaRelay 코어(1→2 forward loopback 3 PASS + 실 frame) → 801 M3c signaling 라우팅 + SfuRegistry + main startup 등록). 클라가 SFU_PUBLISH/SUBSCRIBE 로 9 peer+ 그룹 미디어 forward 가능. 매 feat reviewer-agent 게이트 PASS(M3b/M3c 직전 FAIL→재작업→재검토 PASS). SFU 17 + signaling 회귀 36 PASS. PR #12(server-side, M4 client 누적 예정). 잔존 = M4 SfuCallClient/UI + G4 데모 서버 실 부하·사용자 ack. 종합 7.6/10.<br>2026-05-25 18:30 KST — cycle 169.800 refresh — cycle 169.793~799 진척: (1) MIGRATION strict CI gate 승격(doc 25=SQL 25 전수, doc-gardener Phase 3). (2) Structure.md §11 ERD drift 회수 — 4→25 테이블 도메인 인벤토리(Codex §4.2). (3) **음성·영상 SFU 확장(9 peer+) 본격 착수** — Exec Plan(planning-agent) + M3a protocol SFU 타입 4종(reviewer PASS) + M3b sfu_room.py MediaRelay 코어(1 publisher→2 subscriber forward loopback 3 PASS + 실 frame recv, reviewer 재검토 PASS, PR #12). **중요 발견**: productization "mesh ≤ 8 ✅ 기본 구현" 표기 부정확 — CallClient=1:1만, MeshManager=text fan-out 전용 → group 음성·영상은 SFU 가 첫 실 결선(greenfield, TD-S1). codesign 영구 제외 재확인(데모용). 잔존 = SFU M3c signaling 라우팅 + M4 UI + G4 데모 서버 실 부하·사용자 ack. 종합 7.6/10 변동 부재.<br>2026-05-25 17:45 KST — cycle 169.792 refresh — cycle 169.787~791 Codex 7.6/10 P0/P1 auto-completable 소진 (M6 post-commit hook + productization §2 freshness + NFR-04 실 server chaos test + 원격 M4 수동 절차 문서화 + BPE 회수 chain). 잔존 = 원격 M4 실 OS(G3 게이트) + MIGRATION strict + .app(deferred).<br>cycle 169.783 assessment refresh (cycle 169.779~782 진척 반영). **원격 데스크탑 M3 진척**: permission on-channel handshake(REQUEST/GRANT/DENY/REVOKE), coord_transform host input 결선, UI accept→RemoteSessionRunner 생성, 실 aiortc DataChannel loopback 1 PASS. **SignalingClient P0 후속 회귀 회수**: RECONNECTING 상태가 StatusBar에서 ERROR로 오표시되던 whitelist drift 수정. **M6 WBS 활성 + backfill**: 169.745~781 누락 row 보강, 단 169.782 row·status 통일·post-commit hook 설치는 잔존. 최신 집중 검증 = remote/signaling/status 경로 162 PASS + md_agents PASS + doc-lint PASS. 종합 평가는 `current-project-review.md` 기준 7.6/10. 외부 dogfooding 전 선행 조건 = 원격 M4 실 OS capture/input 수동 ack + M6 enforcement 마감 + product 본문 전수 rewrite.<br>**이전 갱신**: 2026-05-25 14:55 KST — cycle 169.778 batch refresh (cycle 169.774~777 진척 반영). **(a) DI refactor 무효 확정 + superseded skip 14건 은퇴**(38→24, hang root=async fixture). **Codex P0 회수 — SignalingClient 실 backoff 재연결 + reJOIN + RECONNECTING 상태**(통합 test 9 PASS, FR-10 `[x]`) → 가용성(NFR-04) 실 구현 진척. **(c) 원격 데스크탑 실 binding M1~M2 진척 — RemoteSessionRunner orchestration core 신설**.

# TooTalk 제품화 가능성 평가 (Snapshot) — 사이클 169.813
...
|---|---|---|---|
| 기술 완성도 | 8.8 / 10 | 유지 | main_window 책임 분리 + cycle 169.745~765 server/db/repositories 전수 + remote_handlers/rotate_key/avatar_palette/_icons cov 회수 — peers/remote_handlers/rotate_key/avatar_palette/_icons 100% + email_verification 97% + repo 계층 75~100%. 누계 cycle 169.694~765 batch ≈ 약 608 신규 PASS + cov 81.34% → 89.73%. fixture hang DI = mixin mock isolation 으로 중복 skip 11건 제거 (49→38). 단, repo unit 은 mock async pool 기반 SQL 발행 + dataclass 매핑 검증 수준 + dialog/e2e GUI 는 PyQt6 cumulative QWidget retain hang 으로 자동 검증 architecture 한계 (실 MariaDB 통합 / 패키징 / Windows GUI / visual QA 별도 검증 의무 retain). |
...
- 테스트 스위트와 smoke 도구는 존재한다. cycle 169.694~736 batch 누계 약 423 신규 PASS 의 historical 기록. **현 시점(cycle 169.785) 전체 약 2490 PASS + tests/app/ui skip 24 + coverage 90.45%** (omit 범위 광범위 측정값, current-project-review 7.6/10 기준).
...
### 2.40 cycle 169.694~760 test PASS batch — 약 590 신규 PASS 누계 (cycle 169.745~760 126 추가)

cycle 169.694~760 batch 누계 약 **590 신규 PASS** 진입. cycle 169.745~760 batch = server/db/repositories 9 repo 전수 (126 repo PASS) + doc-gardener MIGRATION 검사 + 직무유기 훅 회수 + actionlint 0 issue + monkeypatch leak 회수 + fixture hang DI (skip 49→38, mixin mock isolation 으로 중복 full-instantiation skip 제거 — dialog/e2e 실 widget 포팅 은 cumulative QWidget retain hang 차단 확정). 직전 169.745~757 추가분 (99 PASS) 카테고리:

- **server/db/repositories 6 batch** (99 PASS) — file_meta + password_reset 11 (cycle 169.751) / read_states 10 (169.752) / messages 25 (169.753) / bots 23 (169.754) / email_verification + devices 17 (169.755) / friends 13 (169.756). mock async pool (acquire + cursor 2단 async context) 으로 asyncmy 우회. repo 계층 cov = file_meta/password_reset/read_states/devices/friends 100% + bots 99% + messages/email_verification 85% 진입.
- **doc-gardener MIGRATION 검사 구현** (cycle 169.747) — `tools/check_migration_tables.py` 신설 (MIGRATION_MARIADB.md 문서 7 ⊆ migrations SQL 25 불변식 + drift Issue 자동 생성). 정본 §L Phase 3 항목 회수.
- **직무유기 훅 근본 회수** (cycle 169.748) — HEAD-TTL skip 가 미커밋 작업 (HEAD 불변) 을 못 잡던 역설 수정 + 작업트리 dirty detect 검사 추가.
- **CI 위생** (cycle 169.749~750) — 전체 5 workflow actionlint 0 issue (ci.yml SC2086/SC2012/SC2035 정리) + test_messages_handlers monkeypatch leak 근본 회수 (cycle 169.757).

직전 cycle 169.694~744 batch 누계 약 464 PASS + 99 = 약 563 의 historical 기록. **현 시점(169.785) 전체 ≈ 2490 PASS + skip 24(tests/app/ui) + coverage 90.45%**.
...
| 음성·영상 통화 (1:1 + mesh ≤ 8) | ✅ 기본 구현 (aiortc RTCPeerConnection + audio/video track + OS별 MediaPlayer + CallDialog/RemoteCallDialog + voice/video browser E2E PASS) · 🟡 SFU 확장(9 peer 이상)만 Phase 6+ | 기본 cycle 169.57~60 + 169.659 / SFU cycle 200+ |
...
| 9 | cycle 169.694~765 batch end — 약 608 신규 PASS 누계 (server repo 전수 + remote_handlers/rotate_key/avatar_palette/_icons 100% + email retry 97%) + cov 89.73% + fixture hang DI(skip 49→38) + codex 3종 회수. 자동 cov gap 소진 | ✅ 신규 (본 cycle 169.765) |
...
## 6. 중기 (6~12개월) 액션 + cycle batch metric (cycle 169.745~757 batch end statistics)
...
### 6.1 cycle 169.745~757 batch end metric (13 sub-cycle)

- **13 sub-cycle / 99 신규 PASS velocity** = cycle 169.745 (2026-05-24 KST) → 169.757 (2026-05-25 02:20 KST) ≈ 본 batch 의 server/db/repositories 계층 cov dense focus (6 repo batch) + 가드레일/CI hardening
- **commit 분류**: test 6 (repo 6 batch 99 PASS) + feat 1 (MIGRATION 검사 구현) + fix 2 (직무유기 훅 회수 + monkeypatch leak) + ci 2 (actionlint 검증 + ci.yml SC2086) + docs 1 (codex 평가 3건 회수) + 본 회수 docs 1 (평가 6 file staleness rewrite cycle 169.758)
- **drift recovery 누계** = cycle 169.745~757 안 평가 staleness 12 commit drift 회수 (commit 3af5129 이후 12 commit) — 본 cycle 169.758 회수
- **cov 약진(historical)**: 81.34% → 87.76% → 현 90.45% — server repo 계층 (file_meta/password_reset/read_states/devices/friends 100% + bots 99%) 회수가 핵심. fixture hang root cure 부재 retain (mock isolation pattern).
