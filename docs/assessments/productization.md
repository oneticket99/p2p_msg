...
owner: oneticket99
last_verified: 2026-06-09T16:56:00+09:00
status: active
---

> **최신 갱신 시점**: 2026-06-09 11:39 KST — cycle 169.857 — **mail.dopa.co.kr Dovecot+IMAP 수신 메일박스 신설 M1~M5 코드 완결**. 사용자 directive "mail.dopa.co.kr 계정 추가" + "기존 워크플로우대로 문서부터 만들고 작업을 진행하되 e2e 까지 고려" 정합. 현 인프라 정독 = Postfix outbound only(SASL noreply@) + Dovecot 부재 → 수신/메일박스 미지원 확인. M1 Exec Plan(`docs/exec-plans/active/2026-06-09-dovecot-imap-install.md` 329 line — virtual mailbox 모델 + Postfix LMTP 통합 + passwd-file 계정 store) + M2 `tools/dovecot_install.sh`(270 line Rocky 9.7 idempotent — dnf install dovecot+pigeonhole / vmail uid=5000 / maildir root + chmod 750 명시 / Dovecot 6 conf 파일(dovecot.conf protocols + 10-mail mail_location + 10-auth disable_plaintext + auth-passwdfile.conf.ext SHA512-CRYPT + 10-ssl Let's Encrypt 재사용 + 10-master LMTP unix socket) / passwd-file / Postfix virtual_mailbox_domains + virtual_transport=lmtp:unix:private/dovecot-lmtp / iptables 143+993+4190 / systemctl) + M3 `tools/mail_user_add.sh`(101 line — 인자 형식 검증 `[a-zA-Z0-9._-]+` + 중복 검사 exit 2 + openssl rand 24 byte + doveadm pw SHA512-CRYPT + maildir 3 디렉토리 + printf 패스워드 pipe 보안 강화) + M4 `tests/integration/test_dovecot_imap_e2e.py`(230 line 7 test = 6 PASS + 1 SKIP — IMAP 993 live unreachable expected) + M5 `.env.example` IMAP_* 5 keys + README/History prepend. reviewer FAIL 4 회수(chmod 명시 + printf builtin + test 개수 정합 + Exec Plan §4.4 갱신) 후 재검토 PASS. doc-lint PASS 87 file. PR #28 `feat/cycle169.857-dovecot-imap-install` 생성. SSH 수동 G-final 대기(scp 2건 + install chain + mail_user_add.sh `<user>` + swaks/imaplib 검증). 본 cycle 은 mail server 인프라 신설(IMPLEMENTED) — 실 서버 활성 전까지 VERIFIED 아님. 종합 7.6/10 유지(외부 readiness 무변동, infra 추가).<br>2026-06-09 11:12 KST — cycle 169.856 — **원격 origin/main 143 commit fast-forward pull catchup 로컬 sync**. 신규 작업 머신 `/Users/1ticket/Documents/vscode_work/p2p_msg/` 가 원격 main 대비 143 commit 뒤(cycle 169.849~855 시리즈 전수)였고 `git pull --ff-only` 단일 명령 fast-forward sync(HEAD=`e7d0f53`, working tree clean). 평가 2종 + HTML mirror 4종 marker bump + token-usage-30d 재 산출(window_end 2026-05-27→2026-06-09, sessions 4→9, msgs 22307→27447, cost \$24217→\$28889, cache_hit 96.96%→97.46%). 신규 feat 작업 부재 — pull catchup 단독. PR #27 `chore/cycle169.856-assess-tokenusage-sync` 생성. 종합 7.6/10 유지.<br>2026-05-28 10:20 KST — cycle 169.855 — **한글 주석 상세화 페이즈 M7 server 15/48 진척(batch s2~s5) + test 24/256**. M6 app/ui 51/51 + e2e 9/9 완료 위에서 **M7 server test batch 누적** — avatars_repo/bot_handlers/bots_repo/config/contacts_app_versions_repo/devices_handlers/email_verification(devices_repo+retry)/emoji_moderation_handlers/emoji_packs_schema/file_meta_password_reset_repo/folder_handlers_integration 12 파일 보강(Python filler `한글 주석` prefix 전환 + cycle 169.852 최신 파일 무변경 검수 + `test_config.py` line 45 주어 누락으로 조사 "의"만 남은 잔재 → "DB_PORT invalid int" 의미 복원 + "환경 의 의무 키"/"미테스트 의 의도" false-positive 명확화). server test 는 browser/network 무관이라 실 pytest 가 oracle — s2 81 + s3 64 + s4 29 + s5 35 passed 전수 PASS. 잔여 server 33 + app 155 + G-final(주석 표준 정착 + 전 영역 diff-0 사용자 ack). 전수 기능 diff 0(verify_comment_only AST 동일) + BPE/대명사/이중조사 0(런타임 label "Daisy 의 MacBook" 등 AST Constant 보존). 누계 M2 21+M3 19+M4 16+M5 8+M6 51+M7 24. 신규 VERIFIED capability 부재(주석 = 가독성 품질) — 종합 7.6/10 유지.<br>2026-05-28 09:10 KST — cycle 169.855 — **한글 주석 상세화 페이즈 M6 app/ui 51/51 완료 + M7 test 진입(e2e 9/9 완료 + server 3/48, test 12/256)**. M6 app/ui mixin 22 + dialog 29 전수 완료 후 **M7 test 영역 진입** — 사용자 directive "당연히 e2e 부터 모두 끝내고 사용자 테스트" 정합으로 **M7 순서 e2e(9)→server(48)→app(155) 재배치**. e2e 9/9 전수 완료(conftest SPDX + 전략 line + Python filler 전환, **Playwright page.evaluate() JS 문자열 내 `//` 주석은 AST Constant 라 diff-0 보존**) + server s1 3 파일(bot_escalations/user_activity/auth_handlers_audit, 실 pytest 48 passed). 잔여 server 45 + app 155 + G-final(사용자 ack). 전수 기능 diff 0(verify_comment_only AST 동일) + e2e collect 무결성 + server 실 pytest PASS + offscreen 1963 passed 무변경 + BPE/대명사/이중조사 0(런타임 string false-positive 보존). 누계 M2 21+M3 19+M4 16+M5 8+M6 51+M7 12. 신규 VERIFIED capability 부재(주석 = 가독성 품질) — 종합 7.6/10 유지.<br>2026-05-28 07:40 KST — cycle 169.855 — 한글 주석 페이즈 M6 mixin 22/22 + dialog 24/29(누계 46/51). dialog 계층 §E + 책임 경계 명문화 + filler 전환. 전수 diff-0 + offscreen 1963 passed 무변경. 종합 7.6/10 유지.<br>2026-05-28 06:00 KST — cycle 169.855 — 한글 주석 페이즈 M6 mixin 22/22 완료 + dialog 9/29(누계 31/51). dialog 계층 §E + filler 전환. 전수 diff-0 + offscreen 1963 passed 무변경. 종합 7.6/10 유지.<br>2026-05-28 04:30 KST — cycle 169.855 — 한글 주석 페이즈 M6 app/ui mixin 18/22 진행(batch b1~b6, 누계 M6 18/51). 18 mixin module 계층 §E(MainWindow MRO 합성) + filler 전환. 전수 diff-0 + offscreen 1963 passed 무변경. 누계 M6 18. 종합 7.6/10 유지.<br>2026-05-28 02:10 KST — cycle 169.855 — 한글 주석 페이즈 M5 app/rtc 8/8 완료 + M6 app/ui 3/51 진행. M5 = rtc 8 파일 상태 머신/DataChannel 계약/메모리 release 의도 명시. M6 = ui mixin 3(friend_status/lifecycle_events/signaling) module 계층 §E. 전수 diff-0 + app pytest 무변경. 누계 M2 21+M3 19+M4 16+M5 8+M6 3. 종합 7.6/10 유지.<br>2026-05-28 00:15 KST — cycle 169.855 — **한글 주석 상세화 페이즈 M4 app/net 16/16 전수 완료**. client net 계층 16 파일(b1~b6) module 5요소(역할/계층 §E/의존성/범위/카탈로그) + filler `한글 주석` prefix 전환 + 카탈로그 정정(folder worker 4→5) + QThread worker 생명주기·signal 계약·SSL 우회·async httpx vs urllib 변이 명시 + 이중조사 다수·self-지칭 대명사·BPE U+CE21 단독(UI 쪽) 정정 + signaling SPDX header 추가(타 net 전부 보유 본 파일만 부재). 전수 기능 diff 0(verify_comment_only AST 동일) + app net 93 passed 무변경 + BPE/대명사/이중조사 0. M2 21/21 + M3 19/19 + M4 16/16 완료, 잔여 M5 rtc/M6 ui/M7 test. 신규 VERIFIED capability 부재(주석 = 가독성 품질) — 종합 7.6/10 유지.<br>2026-05-27 22:35 KST — cycle 169.855 — **한글 주석 상세화 페이즈 M4 app/net 9/16 진행 + 다음 세션 인계 자료 재작성**. M4 client net 계층 보강(b1~b3) — _ssl_util/group_message/push/folder/message_protocol/reactions/auth/account/sfu_call 9 파일에 module 5요소(역할/계층 §E/의존성/범위/카탈로그) + filler `한글 주석` prefix 전환 + 카탈로그 정정(folder worker 4→5) + QThread worker 생명주기·signal 계약·SSL 우회·async httpx vs urllib 변이 명시 + sfu self-지칭 대명사 2곳 정정. 잔여 6 = avatars·call·rooms·friends·messages·signaling. 전수 기능 diff 0(verify_comment_only AST 동일) + app pytest 무변경 + BPE/대명사/이중조사 0. 인계 자료 종합 재작성(M2 21/21 + M3 19/19 + M4 9/16 단일 인계점). 신규 VERIFIED capability 부재(주석 = 가독성 품질) — 종합 7.6/10 유지.<br>2026-05-27 20:55 KST — cycle 169.854 — **한글 주석 상세화 페이즈 M3 server API handler 19/19 전수 완료**. blast radius 역순 M3 전수(b1~b8 8 batch) — health/push/read/remote/reactions/contacts/folder/version/avatars/bot_directory/devices/emoji/emoji_moderation/bot/streaming_oauth/rooms/friends/messages/auth. module docstring(역할/계층 §E/의존성/범위/카탈로그) + endpoint 인증·검증 순서·오류 코드·부작용 + filler `한글 주석` prefix 전환. **카탈로그 컨벤션 정정 다수**(실 심볼명+개수): rooms 6→7·friends 6→8·auth 5→15·folder 4→5·read 2→3. 전수 기능 diff 0(`verify_comment_only.py` AST 동일) + server 642 pytest 전량 무변경 + BPE/대명사/이중조사 0(docstring/comment 한정). 런타임 string(streaming HTML·auth register log) 미변경 보존(diff-0 규율, 별도 기능 cycle 백로그). 신규 VERIFIED capability 부재(주석 = 가독성 품질, 외부 readiness 무변동) — 종합 7.6/10 유지.<br>2026-05-27 16:10 KST — cycle 169.853 — **한글 주석 상세화 페이즈 M2 server repository 21/21 완료 + 다음 세션 인계 자료**. blast radius 역순 M2 전수 완료 — friends 본보기(reviewer PASS) + 잔여 20 repo(peers/read_states/password_reset/file_meta/device_tokens/user_contacts/avatars/streaming_oauth_tokens/users/rooms/messages/folders/email_verification/bots/devices/app_versions/bot_escalations/user_activity/emoji_packs/__init__). module/class/함수 docstring(§4 D-1~D-6) + 카탈로그 컨벤션(실 심볼명+개수 일치, rooms/friends/bot_escalations 부정확 카탈로그 정정) + inline "왜". 전수 기능 diff 0(`verify_comment_only.py` docstring 제거 후 AST 동일) + server 642 pytest 무변경 + BPE/대명사/이중조사 0. 인계 자료(`docs/exec-plans/active/2026-05-27-session-handoff-cycle169.853.md`) — 다음 = M3 `server/api/*_handlers.py` 19 파일. 신규 VERIFIED capability 부재(주석 = 가독성 품질, 외부 readiness 무변동) — 종합 7.6/10 유지.<br>2026-05-27 03:30 KST — cycle 169.853 — **한글 주석 상세화 페이즈 M2 진행(server repository 13/21)**. blast radius 역순 M2 server repository 보강 — friends 본보기(reviewer PASS) + peers/read_states/password_reset/file_meta/device_tokens/user_contacts/avatars/streaming_oauth_tokens/users/rooms/messages/folders 누계 13/21. 매 batch 기능 diff 0(verify_comment_only docstring 제거 후 AST 동일) + 관련 pytest 무변경 + BPE/대명사/이중조사 0. rooms/friends 의 부정확 함수 카탈로그(약칭·개수 불일치)도 실 심볼명 기준으로 정정(카탈로그 컨벤션). 주석 전용 트랙 = **코드 동작 변경 0**(ValueError 런타임 string 도 미변경 — diff-0 도구가 검출·차단). 잔여 repo 8 + M3 handler/M4 net/M5 rtc/M6 ui/M7 test(e2e). 신규 VERIFIED capability 부재(가독성 품질) — 종합 7.6/10 유지.<br>2026-05-27 01:30 KST — cycle 169.852~853 — **avatar 이미지 picker M1~M7 전 milestone 완결 + 한글 주석 상세화 페이즈 착수**. (852) avatar M7 문서 동기(Structure/FRONTEND §16.4/ARCHITECTURE §6/CheckList + HTML mirror) + G-final 사용자 수동 검증 체크리스트 §14.1 — avatar M1~M7(서버 영속 + picker + 3 dialog+room kind + 카메라 + 표시 전파 6 site + 문서) 코드+문서 완결, 잔존 = G-final 실 webcam visual ack(사용자 직접). (853) 한글 주석 상세화 페이즈 GO(사용자 directive "e2e 테스트 파일까지 모두 보강") — Exec Plan active 전이 + M7(test e2e 포함) scope 편입 + `tools/verify_comment_only.py`(기능 diff 0 검증, docstring 제거 후 AST 비교) + M1 본보기 friends.py(reviewer PASS, HIGH 회수) + M2 batch-1(peers/read_states/password_reset). **주석 전용 트랙 = 코드 동작 변경 0**(verify_comment_only 전 파일 PASS + pytest 전량 무변경). 신규 VERIFIED capability 부재(주석은 가독성 품질, 외부 readiness 무변동) — 종합 7.6/10 유지.<br>2026-05-26 23:55 KST — cycle 169.852 — **avatar 이미지 picker M6 완결 — 표시 전파 6 site**. M6 T-16 `AvatarCache` 싱글톤(memory+disk 2단 캐시 + `AvatarFetchWorker` async fetch dedup + `avatar_ready` signal + content-addressed 불변 키 + path traversal 방어 + negative-cache) + `make_avatar_pixmap(name, avatar_ref, size)` thin API → T-17 6 site 전파(group/channel chat-list delegate · member_list · drawer header · profile/my_account · chat sender=이름 label N/A · my_profile picker). 업로드 PASS 시 cache seed_image(round-trip 없이 즉시 표시) + drawer header 라이브 갱신. 이니셜 fallback 무손상(빈 avatar_ref = 기존 동작). reviewer-agent T-16 PASS(차단 0) + T-17 PASS(차단 0, MEDIUM 2 비차단 — member row signal disconnect 백로그 + seed emit 의도). 전체 2605 passed(server 642 + app 1963) 회귀 0. avatar M1~M6(서버 영속 + picker + 카메라 + 표시 전파) 코드 완결 — M7 문서 동기 + G-final(실 webcam visual ack, headless 대체 불가) 잔존. 신규 VERIFIED capability 부재(실 OS 카메라/표시 visual ack 전) — 종합 7.6/10 유지.<br>2026-05-26 22:30 KST — cycle 169.852 — **avatar 이미지 picker M5 카메라 — CameraCaptureDialog(QtMultimedia in-app 모달) + picker 연결**. `app/ui/_camera_capture_dialog.py` 신설 — `QCamera`/`QMediaCaptureSession`/`QImageCapture`/`QVideoWidget` live preview + 촬영 → QImage emit. FRONTEND §16 in-app 모달(`FramelessWindowHint` + `exec_modal` parent-walk). graceful 3분기(QtMultimedia ImportError / 카메라 부재 / errorOccurred 권한 거부 → `_show_unavailable` 촬영 button 비활성 crash 차단). 자원 해제(feedback_objc_memory_release) — 종료 3경로(accept/reject/closeEvent) 전부 `_release_camera`(stop + setActive(False) + QObject 3종 deleteLater + 참조 None 이중호출 무crash). picker `_on_camera` 연결("카메라에서" → camera_requested.emit 후방 호환 + CameraCaptureDialog exec_modal → Accepted 시 captured_image null 가드 → `_apply_image`). test_camera_capture_dialog.py 5 PASS(`_init_camera` monkeypatch unavailable 강제 — 실 webcam 미오픈 결정적 + LED/누수 차단). 회귀 fix — 기존 test_camera_action_emits_signal(M3 action trigger)이 M5 핸들러 실 카메라+blocking 모달 진입으로 full-suite hang → stub monkeypatch 차단(signal emit 검증 보존). 전체 2590 passed(server 642 + app 1948) 회귀 0. reviewer-agent PASS(차단 0, OBS 3 비차단 — OBS-1 deleteLater 보강 반영). avatar M6 표시 전파/M7+G-final(실 webcam visual ack) 잔존 — 신규 VERIFIED capability 부재 — 종합 7.6/10 유지.<br>2026-05-26 20:45 KST — cycle 169.852 — **codex 평가 §4.6 하드코딩 수렴 + 한글 주석 보강 페이즈 계획 + Codex 문서 취합**. (1) codex 전면평가 §4.6 환류 — 운영 demo IP routing literal `"http://114.207.112.73:8765"` 8 파일 중복을 `config.DEMO_FALLBACK_API_BASE` 단일 상수로 수렴 + `test_no_443_hardcode` scan gate CI lock(silent wrong-endpoint 502 재발 차단, cycle 823 전례). 값 동일성 + 회귀 0 + reviewer 차단 0. (2) 한글 주석 보강 별도 페이즈 Exec Plan 신설(planning-agent, draft — M1 표준~M6 영역별 + G-final 기능 diff 0). (3) Codex 문서 batch 취합(current-project-review §4.6 + token-usage 5/24~26 필터 + ledger) dirty 해소. avatar M4 클라 dialog 통합/M5 카메라/M6 표시 전파/G-final 잔존 — 신규 VERIFIED capability 부재 — 종합 7.6/10 유지.<br>2026-05-26 20:10 KST — cycle 169.852 — **avatar 이미지 picker M3 클라 picker + M4 서버 chain e2e**. M3 `AvatarPickerButton` 공유 컴포넌트(QMenu 드롭다운 파일/카메라/클립보드, 이모지 제외 + center 정사각 crop 원형 preview + 이니셜/camera fallback) + `avatars_client` QThread worker 3종(upload multipart/fetch/patch_me) + camera/image SVG — 17 PASS, reviewer 차단 0. M4 서버측 — room create `avatar_ref`/`name` 영속 + invite endpoint isolated e2e 보강(create+avatar 2 신규 + invite/kick 8 = 18 PASS). 클라 dialog 통합(M4 잔여)/카메라(M5)/표시 전파(M6)/G-final 잔존 — IMPLEMENTED 진행 중, 신규 VERIFIED capability 부재 — 종합 7.6/10 유지.<br>2026-05-26 19:30 KST — cycle 169.852 — **avatar 이미지 picker M1+M2 — 서버 avatar 영속** (Exec Plan ② 개발, 사용자 GO). 사용자 directive(텔레그램 정합 — 그룹/채널/프로필 3곳 아바타 클릭 → 파일/카메라/클립보드, 이모지 제외, 서버 영속). M1 migration 0018 `users.avatar_ref`(5요소 comment) + `avatars.py`(content-addressed 디스크 저장 + sha256 dedup + `\A..\Z` path traversal 방어) + `users.py`. M2 POST/GET/PATCH endpoint(multipart + Pillow 정사각 512 center crop + EXIF strip) + rooms avatar_ref + route 등록. repo 18 + e2e 17 = 35 PASS, EXIF strip 실측 검증, 전체 2561 passed 회귀 0, reviewer 게이트 2 PASS(차단 0). 서버 avatar 파이프라인 종단 기능 — 클라 picker(M3~M5)/표시 전파(M6)/G-final visual ack 잔존이라 IMPLEMENTED 진행 중, 신규 VERIFIED capability 부재 — 종합 7.6/10 유지.<br>2026-05-26 17:35 KST — cycle 169.850~851 — **codex §8 잔여 auto-completable 전건 회수 + 평가 staleness sweep**. (850) `server/sfu_room.py` 단위 coverage 49→100%(`tests/server/test_sfu_room.py` 10 PASS — MediaRelay forward/재구독 close/remove_peer 양측/close 전수 mock 분기) + M6 WBS 사용자 ack "재개+backfill" 524 row(total 151→675, cycle 72~849) + productization.html 빈 화면 회귀 회수(상단 marker 주석 닫는 `-->` 드롭 → 문서 전체 흡수 fix). (851) codex §8-5 i18n labels dangling 회수(삭제 `group_chat_view.py` 출처 주석 → live `_chat_header_mixin.py:240`/`main_window.py:367` + orphan key `메시지를_입력하세요` 4 dict drop, i18n test 81 PASS) + token-usage-30d 재산출(누적 $42562.61) + active-plan archive(완료 handoff 4종 `completed/` 이동). reviewer 게이트 전수 PASS(차단 0). 전체 2521 PASS, 회귀 0. codex §8 직접 작업 큐 auto-completable 전건 종결(잔존 = §8-4 배포 smoke manual) — 신규 VERIFIED capability 부재 — 종합 7.6/10 유지.<br>2026-05-26 12:05 KST — cycle 169.848~849 — **마이그레이션 M5b 마감 + codex 평가 §8 직접 작업 큐 회수**. (848) M5b — StackedWidget idx 완전 재번호(`_STACK_GROUP_CHAT` 제거 + MEMBERS 2→1 + FRIENDS 3→2 + `_group_placeholder` 제거) + `group_chat_view.py`/`room_list.py` RoomListWidget 파일 삭제(RoomItem 보존) + `_group_message_client` dead attr 회수 + docstring rewrite + `_current_room_id` None clear + README §2.2 stale 정정. reviewer PASS(차단 0). 병렬 Codex 세션 doc-sync 머지(Structure.md UI tree drift 회수). (849) codex 평가 §8 회수 — §8-1 sqlite `ResourceWarning` 결정적 close(`local_db` atexit) + §8-3 `sfu_call_client.py` 단위 test 18 PASS coverage 14→89%(dispatch/dedup/rollback/answer/producers/close 실효 커버). 전체 2511 PASS, 회귀 0. 마이그레이션 완결 + 부채 회수 + 실효 커버리지 확보 — 신규 VERIFIED capability 부재 — 종합 7.6/10 유지.<br>2026-05-26 10:35 KST — cycle 169.843~847 — **room broadcast → 통합 ChatView 마이그레이션 M3~M6 완결**. (843) M3 room 적재 source-of-truth `_room_list._rooms` → `_rooms_cache` 직접 cache 이전. (844) M4 kind=room 진입 통합 ChatView(idx 0) 통일 — `_on_room_entered` early return 제거 + `_current_room_id` 결선(임계 전환점, GroupChatView 사용자 도달 불가). (845) M5(안전) legacy GroupChatView/`room_entered`/RoomListWidget/`_on_room_entered`/`_on_group_message_send`/`_dispatch_message_chain` 물리 회수(idx 재번호는 M5b 분리, `_member_list` group-management 보존). (846) M5 reviewer 게이트 PASS(차단 0) + 다음 session 인계 자료. (847) M6 통합 room-send mesh+REST 신규 coverage(`TestUnifiedRoomSend` 4 PASS — mesh `broadcast_payload` await + REST `_post_and_resolve(room_id=42)` + room `hide_sender=False` 버블 + 공백 early return). reviewer 게이트 M2/M3+M4/M5/M6 전수 PASS. 전체 2504 PASS, 회귀 0. **통합 ChatView 가 friend/bot/saved/room/group 단일 표시·진입·송신 경로 수렴 완료.** 마이그레이션은 기능 보존 + 부채 회수 단계 — 신규 VERIFIED capability 부재 — 종합 7.6/10 유지.<br>2026-05-26 08:40 KST — cycle 169.839~842 — (839) group-flow isolated test 재구성 — cycle169.838 "방 입장"(room_id 직접 입력) 폐지 정합으로 `test_main_window_rooms.py` 의 구 `room_entered.emit(N)` (GroupChatView idx 1) 방식을 그룹 만들기 wizard chain 으로 전면 교체(통합 ChatView idx 0 canonical 확정, 6 PASS). (840) token-usage-30d 재산출(원격 git `.bak` 병합 + 현 세션 합산, 누적 187억 토큰/$41,954). (841) current-project-review 전면평가 최신화(7.7/10 보수 조정, legacy room path 마이그레이션 P0 승격). (842) **room broadcast → 통합 ChatView 마이그레이션 착수** — planning-agent Exec Plan 6 단계(M1~M6 + G-final 게이트) + M1 재검증(group broadcast inbound 표시 결선 부재 확정, option b) + M2 송신 echo 재배선(`_on_group_message_send` echo 를 legacy `_group_chat_view.append_message` → 통합 `_chat_view.add_message(hide_sender=False, play_sound=False)`, `_dispatch_message_chain` REST+mesh 불변). UI 344 PASS, 회귀 0. 마이그레이션은 기능 보존 단계 — 신규 VERIFIED capability 부재 — 종합 7.6/10 유지.<br>2026-05-26 06:48 KST — cycle 169.838 — 전 dialog in-app overlay 모달 변환 완성(별도 OS 윈도우 제거, 새창=원격제어 상대화면 창 1개뿐): `_exec_dialog_centered`/`_embed_dialog_centered`/`_modal_helper.exec_modal` 3 진입 경로 확립 + ConfirmDialog 정적 헬퍼(얼럿/확인) in-app 화(40+ 호출 사이트 무변경) + signup→OTP nested exec_modal + "방 입장"(room_id 직접 입력) 전수 제거(그룹방=그룹만들기+초대) + ChatHeader stale 수정 + 별도 윈도우 예외 4종 FRONTEND.md §16 명문화. reviewer-gate 2차 PASS(M1 BLOCKER+HIGH 3+MEDIUM 1+OBSERVATION 1 전건 회수). UX 정합 완성도 향상이나 신규 VERIFIED capability 부재 — 종합 7.6/10 유지.<br>2026-05-26 02:40 KST — cycle 169.835~837 — startup 로그인 메뉴 토글 회수(835) + 그룹 멤버 보기 "..." 드롭다운 이동·room kind=group·메시지 수신음 실 파일 교체(836) + 그룹 멤버 UX 완성(멤버 모달 + 원형 아바타 행 통합 + 그룹 메뉴 미구현 stub 전수 제거, 837). 사용자 dogfooding 회수 chain — UX 정합·미구현 노출 제거이나 신규 VERIFIED capability 부재 — 종합 7.6/10 유지.<br>2026-05-26 02:00 KST — cycle 169.834 — dogfooding 버그 6종 배치(채팅 스크롤 중복·로그인후 메뉴 auth·친구 요청/승인 모델·헤더 멤버수·트레이 문구) + user-facing 문구 i18n 5언어(ko/en/zh-CN/zh-TW/ja) 친절화(11 UI) + 메시지 수신음 교체. 친구 요청 흐름·문구 품질·UX 개선이나 신규 VERIFIED capability 부재 — 종합 7.6/10 유지.<br>2026-05-26 01:30 KST — cycle 169.826 marker 동기 — 데모 서버 502 회수(SFU aiortc graceful optional import → 코어 시그널링/인증/메시지 부팅 보장, web/ws crash loop 해소 + 데모 서버 deployability 복구, main `5ea8b2e` PR #17 merge). 502 회수는 기존 기능 복구이므로 신규 VERIFIED capability 부재 — 종합 7.6/10 유지.<br>2026-05-26 00:15 KST — cycle 169.824 refresh — 443 nginx 전수조사 + 클라이언트 dead-path 제거(Windows 회원가입 502 회수). 데모 nginx 443 의 web:8080/ws:8765 upstream 컨테이너 다운 502 근본 식별(host-publish 8765 직결만 UP), 클라이언트 포트 없는 https 443 fallback 10건 + 죽은 auto-update :8080 poller 3종 → http 8765 전수 치환 + guard test(PR #16 merge, reviewer 2회 PASS). Codex 외부평가 §3.7(그룹 모델)·§3.8(443 전수조사) 환류 + README/History M1 doc-sync. Windows 재빌드. 서버측 443 정상화(docker web/ws 재기동)는 SSH 게이트 DEFERRED. 종합 7.6/10 유지(외부 readiness 무변동, dead-path 핫픽스).<br>2026-05-25 23:40 KST — cycle 169.821 refresh — 텔레그램 그룹 멤버 관리 write 경로 schema foundation 진입(820 Exec Plan 신설 docs/exec-plans/active/2026-05-25-telegram-group-management.md 5 화면 모델→REST→UI M1~M5 분해 + 821 migration 0017 peers.role ENUM owner/admin/member 3-tier + rooms name/description/avatar_ref 그룹 메타 컬럼 + isolated test 15 + MemberPanel member_count/viewer_role 위임 CI 회귀 회수). 819 그룹 멤버 보기 UX 회수(MemberListWidget→MemberPanel 헤더 래퍼 + 빈 화면 populate). 5단계 워크플로우 전수 PASS(reviewer→qa→observability) + PR #15. SFU 그룹 통화 종단 IMPLEMENTED 유지. 신규 VERIFIED capability 부재 — 종합 7.6/10 유지.<br>2026-05-25 22:10 KST — cycle 169.818 refresh — Codex 평가 환류 완료(811~816: review checker PASS + 과거 표현 sweep + productization 본문 전수 rewrite + FR 추적표 per-file 감사) + 빌드 테스트 회수(817: 로그인 HTTP 502 → Config.api_base http 8765 single source + confirm_dialog 배경 투명). macOS .app 정상 기동 검증 + Windows CI 빌드. SFU 그룹 통화 종단 IMPLEMENTED 유지. 종합 7.6/10.<br>2026-05-25 21:00 KST — cycle 169.815 전수 rewrite — Codex 전면평가 §4.2 환류로 §2 강점 본문의 cycle-by-cycle 역사 로그를 현 상태 중심 prose 로 압축. 음성·영상 SFU 그룹 통화(9 peer+) 종단 코드 완결 반영(server PR#12 + client PR#13 merge, MainWindow entry 까지). reviewer-gate 11 feat 전수 PASS + headless aiortc forward + offscreen Qt 검증. SFU 는 IMPLEMENTED 단계 — 실 OS 미디어 캡처 + 다중 화면 visual ack 전까지 VERIFIED 아님. 종합 7.6/10 유지.

# TooTalk 제품화 가능성 평가 (Snapshot) — 사이클 169.862
...
> 평가 기준일 = 2026-06-09. 평가 범위 = 본 저장소 p2p_msg / TooTalk 프로젝트 cycle 169.862 누계 (main HEAD `31ff549` — 6 PR squash merge 완료 #28~#33 + 실 서버 webmail.dopa.co.kr M4 IMAP 결선 종단 작동).
...
이번 cycle(169.855)의 진행은 **한글 주석 상세화 페이즈 M7 server 15/48 진척(batch s2~s5) + test 24/256**이다(M2 repo 21 + M3 handler 19 + M4 net 16 + M5 rtc 8 + M6 ui 51 + e2e 9 완료 위). M6 app/ui(mixin 22 + dialog 29) 전수 완결 + e2e 9/9 후 **M7 server test batch**로 진입해 s2~s5 누적 12 파일(avatars_repo/bot_handlers/bots_repo/config/contacts_app_versions_repo/devices_handlers/email_verification 2종/emoji_moderation_handlers/emoji_packs_schema/file_meta_password_reset_repo/folder_handlers_integration)을 보강했다. test 파일 작업에서 중요한 변별 = **Playwright `page.evaluate()` JS 문자열 안 `//` 주석은 AST Constant 라 diff-0 보존**(Python 주석만 전환) + **런타임 label string("Daisy 의 MacBook" 등)도 AST Constant 라 보존**(comment 의 조사 "의"와 구분). 추가로 `test_config.py` line 45 의 **주어 누락으로 조사 "의"만 남은 filler 잔재를 "DB_PORT invalid int → default 3306 fallback"로 의미 복원** + "환경 의 의무 키"/"미테스트 의 의도" false-positive 를 docstring/comment 명확화로 정정했다. server test 는 e2e 와 달리 browser/network 무관이라 실 pytest 가 oracle(s2 81 + s3 64 + s4 29 + s5 35 passed 전수 PASS). 잔여 server 33 + app 155 + G-final(주석 표준 정착 + 전 영역 diff-0 사용자 ack). 주석 전용 품질 트랙이라 `verify_comment_only.py`(docstring 재귀 제거 후 AST 동일) 전수 PASS + e2e collect 무결성 + server 실 pytest PASS + offscreen(QT_QPA_PLATFORM=offscreen) 1963 passed 무변경(코드 동작 변경 0). 런타임 string(streaming_oauth HTML 응답·auth register log)은 diff-0 규율상 미변경 보존. avatar 잔존은 **G-final 실 webcam visual ack**(headless 대체 불가, 사용자 직접)뿐이다.

SFU 는 **IMPLEMENTED 단계이지 VERIFIED 아니다** — 실 OS 미디어 캡처 + 다중 화면 사용자 visual ack 가 끝나야 검증 완료로 본다. avatar/카메라 역시 실 하드웨어 visual ack 전까지 IMPLEMENTED 다. 같은 이유로 점수는 7.6/10 으로 유지한다(picker/SFU 결선·주석 보강은 차별화 방향·가독성 품질 증거이나 외부 readiness 지표는 아니다). 테스트는 약 2605 PASS(server 642 + app 1963) + coverage 약 90% (omit 범위 광범위). 제품화 readiness = **내부 dogfooding 후보, 외부 배포 미진입**.

| 항목 | 점수 (10점, 0.1 단위) | 직전 → 현재 | 근거 |
|---|---|---|---|
| 기술 완성도 | 8.8 / 10 | 유지 | main_window 책임 분리(21 mixin) + server/db/repositories 계층 전수 unit cov + remote_handlers/rotate_key/avatar_palette/_icons 100% + email_verification 97% + repo 계층 75~100%. 약 2590 PASS(server 642 + app 1948) / cov 약 90%. SFU server+client 종단 코드 + signaling 36 회귀 + SFU 17 PASS + avatar picker M3~M5(picker/upload/camera) test. 단, repo unit 은 mock async pool 기반 + dialog/e2e GUI 는 PyQt6 cumulative QWidget retain hang 으로 자동 검증 architecture 한계 (실 MariaDB 통합 / 패키징 / Windows GUI / 실 OS 미디어 캡처(카메라 포함) visual QA 별도 검증 의무 retain). |
...
| **종합** | **7.6 / 10** | **유지** | **cycle 169.857 — mail.dopa.co.kr Dovecot+IMAP 수신 메일박스 신설 M1~M5 코드 완결 + PR #28 생성. Exec Plan 329 line(virtual mailbox 모델 + Postfix LMTP 통합 + passwd-file 계정 store) + dovecot_install.sh 270 line(Rocky 9.7 idempotent 10 단계, vmail uid=5000 + Dovecot 6 conf + Let's Encrypt 재사용 + iptables 143+993+4190) + mail_user_add.sh 101 line(인자 형식 검증 + printf 패스워드 pipe + sasldb2 동시 자격) + e2e 7 test(6 PASS + 1 SKIP IMAP 993 live unreachable expected) + .env.example IMAP_* keys. reviewer FAIL 4 회수(chmod 명시 + printf builtin + test 개수 정합) 후 재검토 PASS. doc-lint PASS 87 file. 사용자 SSH 수동 G-final 대기 — scp 2건 + install chain + mail_user_add.sh `<user>` + swaks/imaplib 검증. 본 cycle 은 mail server 인프라(IMPLEMENTED) — 실 서버 활성 전까지 VERIFIED 아님. 외부 readiness 무변동 — 점수 변동 부재. [직전 근거 보존] cycle 169.856 origin/main 143 commit pull catchup + 평가/HTML/token-usage staleness 회수 단독(PR #27 생성). 한글 주석 상세화 페이즈 M7 server 15/48 진척(batch s2~s5, cycle 169.855, e8d84ef — 주석 전용 트랙, 코드 동작 변경 0) + 직전 M6 ui 51/51 + e2e 9/9 + M5 rtc 8/8 + M4 net 16/16 + M3 handler 19/19 + M2 repo 21/21 + avatar M1~M7 완료. M7 server = repo/handler test 12 파일 filler prefix 전환 + 무변경 검수(cycle 169.852 최신) + line 45 주어 누락 "의" 잔재 의미 복원("DB_PORT invalid int") + "환경 의 의무 키/미테스트 의 의도" false-positive docstring/comment 명확화. server test 는 browser/network 무관이라 실 pytest oracle — s2 81 + s3 64 + s4 29 + s5 35 passed 전수 PASS. M3 = 카탈로그 컨벤션 정정 다수(rooms 6→7·friends 6→8·auth 5→15·folder 4→5·read 2→3). verify_comment_only diff-0 전수 PASS + server/app pytest 무변경(기능 영향 0). 런타임 string·label("Daisy 의 MacBook") 미변경 보존(diff-0 규율, AST Constant). 다음 = M7 server 잔여 33 → app 155 → G-final. 신규 VERIFIED capability 부재(문서/주석 = 가독성 품질, 외부 readiness 무변동) — 점수 변동 부재. [직전 근거 보존] avatar M1~M6 코드 완결(ab9f505) — 서버 영속(0018 users.avatar_ref + avatars.py content-addressed repo + POST/GET/PATCH) + AvatarPickerButton 3 dialog 통합 + 서버 room kind(0019 group/channel) + CameraCaptureDialog(QtMultimedia in-app 모달) + AvatarCache 싱글톤(mem+disk 캐시 + async fetch + avatar_ready + negative-cache) + make_avatar_pixmap 표시 전파 6 site(group/channel/member_list/drawer/profile, chat sender=이름 label N/A). 업로드 PASS 시 seed_image 즉시 표시 + 이니셜 fallback 무손상. reviewer-agent T-16 PASS + T-17 PASS(차단 0, MEDIUM 2 비차단). 전체 약 2605 PASS(server 642 + app 1963) 회귀 0. 직전 room broadcast → 통합 ChatView 마이그레이션 M1~M5b(169.842~848) + codex §8 큐(169.849~851) 누계 유지. avatar/카메라/SFU/원격 데스크탑 모두 IMPLEMENTED — 실 OS 미디어 캡처(카메라)/표시 visual ack(M7+G-final) 전까지 VERIFIED 아님. 외부 배포 단계 미진입 retain. 외부 readiness 무변동, 점수 변동 부재.** |
...
- 테스트 스위트 + smoke 도구 존재. **현 시점 약 2605 PASS(server 642 + app 1963) + skip 24 + coverage 약 90%** (omit 범위 광범위 측정값). 한글 주석 페이즈는 기능 diff 0 라 pytest 무변경. cycle 169.849~851 coverage omit 축소 1차 완결 — `sfu_call_client.py` 18 PASS 14→89%(849) + `server/sfu_room.py` 10 PASS 49→100%(850, MediaRelay forward/remove_peer/close 전수 mock 분기) + sqlite ResourceWarning 결정적 close(849) + codex §8-5 i18n labels dangling 출처 주석 정리(851).
...
| room/group 통합 ChatView 단일 경로 (표시·진입·송신 수렴) | ✅ 마이그레이션 M1~M5b 완결 (legacy GroupChatView/room_entered/RoomListWidget 물리 회수 + idx 완전 재번호) |
| avatar 이미지 picker (그룹/채널/프로필 3 dialog + 파일/카메라/클립보드 + 표시 전파) | 🟡 M1~M7 코드+문서 완결 (서버 영속 0018 + AvatarPickerButton + 3 dialog 통합 + 서버 room kind 0019 + 카메라 CameraCaptureDialog + AvatarCache 표시 전파 6 site + 문서 동기/HTML mirror). G-final 실 webcam visual ack(사용자 직접) 잔존 |
| 한글 주석 상세화 페이즈 (소스 + e2e test 전량 docstring/inline 보강) | 🟡 진행 (active) — 주석 전용 트랙(기능 diff 0). verify_comment_only diff-0 검증 도구 + M1 표준 본보기 + **M2 repo 21 + M3 handler 19 + M4 net 16 + M5 rtc 8 + M6 ui 51/51 완료 + M7 test 24/256 진행(e2e 9/9 완료 + server 15/48 batch s2~s5)**(전수 diff-0 + server 실 pytest 무변경 + e2e collect 무결성, 카탈로그 정정 다수, ui 계층 §E + 책임 경계, JS page.evaluate 문자열 `//` 주석·런타임 label diff-0 보존, line 45 주어 누락 "의" 잔재 의미 복원). M7 server 잔여 33 + app 155 + G-final. 코드 동작 변경 0(가독성 품질 트랙) |
...
> cycle 169.862 sweep (webmail 6 cycle batch 종결 — 169.857 Dovecot+IMAP + 169.858 nginx vhost + 169.859 backend skeleton + 169.860 staleness 회수 + 169.861 IMAP 결선 + 169.862 parse 회수, infra 확장 + reviewer 사후 PASS — 제품화 액션 큐 무영향): 액션 큐 우선순위 무변동. 한글 주석 페이즈는 가독성 품질 트랙 — 제품화 액션 항목(Toonation API / OBS / SFU visual ack / 원격 M4 / coturn / mobile)에 영향 0. 사용자 직접 게이트 항목(🔴) 그대로 retain.
...
> cycle 169.862 sweep (webmail 6 cycle batch 종결 — 169.857 Dovecot+IMAP + 169.858 nginx vhost + 169.859 backend skeleton + 169.860 staleness 회수 + 169.861 IMAP 결선 + 169.862 parse 회수, infra 확장 + reviewer 사후 PASS — 제품화 액션 큐 무영향): 중기 액션 무변동. Phase 5 마무리 + dogfooding + SFU/원격 실 OS 검증 + mobile 본격 진입 우선순위 그대로. 주석 페이즈는 코드 동작 변경 0 라 중기 로드맵 비영향.
...
- **테스트**: 약 2605 PASS(server 642 + app 1963) + tests/app/ui skip 24 + coverage 약 90% (omit 범위 광범위). 한글 주석 페이즈는 기능 diff 0 → pytest 무변경.
...
> cycle 169.857 sweep (mail.dopa.co.kr Dovecot+IMAP 신설 M1~M5): 리스크 프로파일 무변동. 본 cycle = mail server 인프라 추가(IMPLEMENTED, 실 서버 활성 G-final 전) + bash 스크립트 reviewer FAIL 회수(chmod 명시 + printf builtin 패스워드 pipe + test 개수 정합) 후 재검토 PASS. 신규 리스크 부재 — fixture hang / 실 OS visual ack 미검증 / 외부 dogfooding 부재 리스크 그대로 retain. 신규 리스크 부재 — 주석 페이즈는 기능 diff 0(verify_comment_only AST 게이트 + server 실 pytest s2~s5 209 passed + offscreen 1963 passed 무변경)라 회귀/장애 표면 증가 0. fixture hang / 실 OS visual ack 미검증 / 외부 dogfooding 부재 리스크 그대로 retain.
