...
| **종합** | **7.6 / 10** | **유지** | **avatar 이미지 picker M5 카메라 완결(cycle 169.852, ef3dffd) — `CameraCaptureDialog`(QtMultimedia in-app 모달) live preview+촬영→QImage + picker `_on_camera` 연결 + graceful 3분기 + 자원 해제 3경로(deleteLater) + test 5 PASS + 기존 action test full-suite hang 회귀 fix. reviewer-agent PASS(차단 0, OBS 3 비차단). M3 picker + M4 서버 room(group/channel kind 0019) + 3 dialog 통합에 이어 카메라 입력 결선 — avatar M6 표시 전파/M7 문서/G-final(실 webcam visual ack) 잔존. 직전 room broadcast → 통합 ChatView 마이그레이션 M1~M5b 완결(169.842~848) + codex §8 큐 회수(169.849~851) 누계 유지. 전체 약 2590 PASS(server 642 + app 1948) 회귀 0. 카메라/SFU/원격 데스크탑 모두 IMPLEMENTED — 실 OS 미디어 캡처/다중 화면 visual ack 전까지 VERIFIED 아님. 외부 배포 단계 미진입 retain. 외부 readiness 무변동, 점수 변동 부재.** |
...
| avatar 이미지 picker (그룹/채널/프로필 3 dialog + 파일/카메라/클립보드) | 🟡 M1~M5 완결 (서버 영속 0018 + AvatarPickerButton + 3 dialog 통합 + 서버 room kind 0019 + 카메라 CameraCaptureDialog). M6 표시 전파/M7 문서/G-final 실 webcam visual ack 잔존 |
