...
이번 cycle(169.852)의 진행은 **avatar 이미지 picker M5 카메라** 다 — `CameraCaptureDialog`(QtMultimedia QCamera/QImageCapture/QVideoWidget in-app 모달) live preview + 촬영 → QImage 회신, picker "카메라에서" 연결, graceful 3분기(ImportError/카메라 부재/권한 거부), 자원 해제 3경로(stop+setActive(False)+deleteLater, objc-release 정합). M3 picker + M4 서버 room(group/channel kind 0019) + 프로필/그룹/채널 3 dialog 통합에 이어 카메라 입력까지 결선됐다. 다만 avatar 표시 전파(M6) + 문서 동기(M7) + 실 webcam visual ack(G-final)이 잔존하고, 카메라는 headless 에서 `_init_camera` monkeypatch 로만 검증(실 하드웨어 캡처는 사용자 visual ack 영역)이라 **IMPLEMENTED 진행 단계**다.

마찬가지로 SFU 는 **IMPLEMENTED 단계이지 VERIFIED 아니다** — 실 OS 미디어 캡처 + 다중 화면 사용자 visual ack 가 끝나야 검증 완료로 본다. 같은 이유로 점수는 7.6/10 으로 유지한다(picker/SFU 결선은 차별화 방향 증거이나 외부 readiness 지표는 아니다). 테스트는 약 2590 PASS(server 642 + app 1948, +M5 5) + coverage 약 90% (omit 범위 광범위). 제품화 readiness = **내부 dogfooding 후보, 외부 배포 미진입**.
...
| 한글 주석 상세화 페이즈 (소스 + e2e test 전량 docstring/inline 보강) | 🟡 착수 (active) — 주석 전용 트랙(기능 diff 0). verify_comment_only diff-0 검증 도구 + M1 표준 본보기 friends.py + M2 server repository 4/21. M3 handler/M4 net/M5 rtc/M6 ui/M7 test(e2e) 잔여. 코드 동작 변경 0(가독성 품질 트랙) |
