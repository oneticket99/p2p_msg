...
- 테스트 스위트 + smoke 도구 존재. **현 시점 tests/app+server 2511 PASS + skip 24 + 전체 약 2546 PASS(integration 포함) + coverage 약 88%** (omit 범위 광범위 측정값). cycle 169.849 — `sfu_call_client.py` 단위 test 18 PASS 신설로 14→89% (SFU dispatch 실효 커버) + sqlite ResourceWarning 결정적 close.
